...
namespace gdwcore {
    namespace client {
        namespace detail {
            ...
            std::vector<gdwcore::consensus::AccountEntry> ClientImpl::wallet_get_all_approved_accounts(int8_t approval)
            {
                // set limit in  simulator state
                if (_chain_db->get_is_in_simulator())
                    FC_THROW_EXCEPTION(simulator_command_forbidden, "in simulator, this command is forbidden, you cannot call it!");
            ...
            void detail::ClientImpl::wallet_create(const string& wallet_name, const string& password, const string& brain_key)
            {
                ...
                if (brain_key.size() && brain_key.size() < GDW_WALLET_MIN_BRAINKEY_LENGTH) FC_CAPTURE_AND_THROW(brain_key_too_short);
                if (password.size() < GDW_WALLET_MIN_PASSWORD_LENGTH) FC_CAPTURE_AND_THROW(password_too_short);
            ...
            void read_keys(const fc::variant& vo, vector<PrivateKeyType>& keys, const string& password)
            {
                ...
                try
                {
                    ...
                    const auto key = gdwcore::utilities::wif_to_key(wif_key);
                ...
                catch (const gdwcore::wallet::invalid_password&)
                {
                    throw;
                }
                catch (...)
            ...
            */
            ...
            vector<gdwcore::wallet::PrettyTransaction> detail::ClientImpl::wallet_transaction_history_splite(const std::string& account_name,
                ...
                int32_t transaction_type)const
            {
                ...
                try {
                    const auto history = _wallet->get_pretty_transaction_history(account_name, 0, -1, asset_symbol, (gdwcore::wallet::Wallet::TrxType)transaction_type, true);
            ...
            map<TransactionIdType, fc::exception> detail::ClientImpl::wallet_get_pending_transaction_errors(const gdwcore::consensus::FilePath& filename)const
            {
                // set limit in  simulator state
                if (_chain_db->get_is_in_simulator())
                    FC_THROW_EXCEPTION(simulator_command_forbidden, "in simulator, this command is forbidden, you cannot call it!");
            ...
            WalletTransactionEntry detail::ClientImpl::sign_build_transaction(const gdwcore::wallet::WalletTransactionEntry& trasaction_building)
            {
			...
			fc::variant_object detail::ClientImpl::wallet_builder_get_multisig_detail(const gdwcore::wallet::TransactionBuilder& transaction_builder) const
			{
				try {
			...
			fc::variant_object detail::ClientImpl::wallet_builder_file_get_multisig_detail(const gdwcore::consensus::FilePath& builder_path) const
			{
				try {
					auto new_builder = _wallet->create_transaction_builder_from_file(builder_path);
			...
			fc::variant_object detail::ClientImpl::wallet_import_multisig_account(
				const gdwcore::consensus::Address& multisig_address)
			{
			...
			Address detail::ClientImpl::wallet_import_multisig_account_by_detail(
				const string& asset_symbol,
				uint32_t m,
				const vector<gdwcore::consensus::Address>& addresses)
			{
			...
			std::pair<std::string, gdwcore::wallet::WalletTransactionEntry> detail::ClientImpl::wallet_create_multisig_account(
				const string& amount,
				const string& symbol,
				const string& from_name,
				uint32_t m,
				const std::vector<gdwcore::consensus::Address>& addresses,
				const Imessage& memo_message)
			{
				Asset ugly_asset = _chain_db->to_ugly_asset(amount, symbol);
				auto builder = _wallet->create_transaction_builder();
			...
			std::vector<gdwcore::wallet::PrettyTransaction> detail::ClientImpl::wallet_multisig_account_history(const std::string& account_address, const std::string& asset_symbol, int32_t limit, uint32_t start_block_num, uint32_t end_block_num) const
			{
				try {
					Address multisig_address(account_address);
					if (multisig_address.judge_addr_type(account_address) != AddressType::multisig_address)
						return std::vector<gdwcore::wallet::PrettyTransaction>();
            ...
            vector<gdwcore::wallet::AccountAddressData> detail::ClientImpl::wallet_list_my_addresses() const
            {
                // set limit in  simulator state
                if (_chain_db->get_is_in_simulator())
                    FC_THROW_EXCEPTION(simulator_command_forbidden, "in simulator, this command is forbidden, you cannot call it!");
            ...
            // vector<gdwcore::wallet::escrow_summary> ClientImpl::wallet_escrow_summary( const string& account_name ) const
            // { try {
            ...
            std::map<std::string, gdwcore::consensus::DelegatePaySalary>  ClientImpl::wallet_active_delegate_salary()
            {
                // set limit in  simulator state
                if (_chain_db->get_is_in_simulator())
                    FC_THROW_EXCEPTION(simulator_command_forbidden, "in simulator, this command is forbidden, you cannot call it!");
            ...
            void ClientImpl::wallet_set_transaction_imessage_fee_coe(const string& fee_coe)
            {
                ...
                try
                {
                    ...
                    if (ipos != string::npos)
                    {
                        ...
                        FC_ASSERT((precision_input <= GDW_BLOCKCHAIN_PRECISION), "Precision is not correct");
                    }
                   
                    Asset asset_to_transfer = _chain_db->to_ugly_asset(fee_coe, GDW_BLOCKCHAIN_SYMBOL);
            ...
            double ClientImpl::wallet_get_transaction_imessage_fee_coe()
            {
                ...
                return ((double)fee_coe) / GDW_BLOCKCHAIN_PRECISION;
            }
            ...
            Asset ClientImpl::wallet_set_transaction_fee(const string& fee)
            {
                ...
                try {
                    ...
                    Asset asset_to_transfer = _chain_db->to_ugly_asset(fee, GDW_BLOCKCHAIN_SYMBOL);
            ...
            Asset ClientImpl::wallet_get_transaction_fee(const string& fee_symbol)
            {
                ...
                if (fee_symbol.empty())
                    return _wallet->get_transaction_fee(_chain_db->get_asset_id(GDW_BLOCKCHAIN_SYMBOL));
            ...
            std::string ClientImpl::wallet_transfer_to_address_rpc(const std::string& amount_to_transfer, const std::string& asset_symbol, const std::string& from_account_name, const std::string& to_address, const gdwcore::consensus::Imessage& memo_message /* = fc::json::from_string("").as<std::string>() */, const gdwcore::wallet::VoteStrategy& strategy /* = fc::json::from_string("vote_recommended").as<gdwcore::wallet::vote_strategy>() */)
            {
                // set limit in  simulator state
                if (_chain_db->get_is_in_simulator())
                    FC_THROW_EXCEPTION(simulator_command_forbidden, "in simulator, this command is forbidden, you cannot call it!");
            ...
            std::string detail::ClientImpl::wallet_transfer_to_public_account_rpc(
                ...
                const string& from_account_name,
                const string& to_account_name,
                const gdwcore::consensus::Imessage& memo_message,
                const gdwcore::wallet::VoteStrategy& strategy)
            {
                // set limit in  simulator state
                if (_chain_db->get_is_in_simulator())
                    FC_THROW_EXCEPTION(simulator_command_forbidden, "in simulator, this command is forbidden, you cannot call it!");
            ...
            string ClientImpl::wallet_account_balance_rpc(const string& account_name)const
            {
                ...
                try {
                    ...
                    else
                    {
                        ...
                        auto resu = re_val.find(gdwcore::consensus::Asset(0, 0).asset_id);
                        if (resu == re_val.end())
                        {
                            res += "0\"}";
                        }
