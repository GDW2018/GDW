    ...
    namespace client {
        namespace detail {
            ...
            vector<string> ClientImpl::blockchain_list_missing_block_delegates(uint32_t block_num)
            {
                ...
                timestamp += GDW_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
                while (timestamp != this_block.timestamp)
                    // if time interval between prev_block and this block greater than GDW_BLOCKCHAIN_BLOCK_INTERVAL_SEC,
            ...
            std::vector<std::pair<gdwcore::consensus::TransactionIdType, gdwcore::consensus::SignedTransaction>>  ClientImpl::blockchain_list_pending_transactions() const
            {
                // set limit in  simulator state
                if (_chain_db->get_is_in_simulator())
                    FC_THROW_EXCEPTION(simulator_command_forbidden, "in simulator, this command is forbidden, you cannot call it!");

                std::vector<std::pair<gdwcore::consensus::TransactionIdType, gdwcore::consensus::SignedTransaction>> trxs;
            ...
            oAccountEntry detail::ClientImpl::blockchain_get_account(const string& account)const
            {
                ...
                try
                {
                    ...
                    if (std::all_of(account.begin(), account.end(), ::isdigit))
                    ...
                    else if (account.substr(0, string(GDW_ADDRESS_PREFIX).size()) == GDW_ADDRESS_PREFIX) {
                        ...
                        if (account.length() < 39)
                            ...
                            return _chain_db->get_account_entry(Address(gdwcore::consensus::PublicKeyType(account)));
                    }
                    else
            ...
            fc::variant_object detail::ClientImpl::blockchain_list_address_transactions(const string& raw_addr, uint32_t after_block)const
            {
                ...
                try {
                    ...
                    if (after_block > 0)
                        ...
                        [after_block](const gdwcore::consensus::TransactionEntry& t) -> bool {
            ...
            {
                // 如果contract_id 传入为空字符串，则代表所有的合约
                // 如果trx_type 传入99，则代表所有的合约交易类型
                // 如果call_method 传入为空字符串，则代表所有的调用合约的交易（前提是trx_type为14）
                // set limit in  simulator state
                if (_chain_db->get_is_in_simulator())
                    FC_THROW_EXCEPTION(simulator_command_forbidden, "in simulator, this command is forbidden, you cannot call it!");
                ...
                FC_ASSERT((trx_type == 99) ||(trx_type >= gdwcore::consensus::TransactionType::contract_register_transaction &&
                    trx_type <= gdwcore::consensus::TransactionType::contract_call_transaction), "invalid transaction type");
                    ...
                    for (; block_num_start <= block_num_end; ++block_num_start)
                    {
                        ...
                        for (auto trx_pair : trx_map){
                            ...
                            if ((trx_entry.trx.operations.size() > 0) && (trx_entry.trx.operations[0].type.value == transaction_op_type))
                            {
                                ...
                                if (trx_type == gdwcore::consensus::TransactionType::contract_call_transaction)
                                {
                                    if (call_method != "")
                                    {
			...
			std::vector<fc::string> ClientImpl::blockchain_list_pub_all_address(const std::string& pub_key) const
			{
				...
				try
				{
					...
					if (fc::string::npos != pub_key.find(GDW_ADDRESS_PREFIX))
					{
						auto pub_type = gdwcore::consensus::PublicKeyType(pub_key);
            ...
            variant_object ClientImpl::blockchain_get_info()const
            {
                ...
                info["name"] = GDW_BLOCKCHAIN_NAME;
                info["symbol"] = GDW_BLOCKCHAIN_SYMBOL;
                info["address_prefix"] = GDW_ADDRESS_PREFIX;
                info["db_version"] = GDW_BLOCKCHAIN_DATABASE_VERSION;
                info["genesis_timestamp"] = _chain_db->get_genesis_timestamp();

                info["block_interval"] = GDW_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
                info["delegate_num"] = GDW_BLOCKCHAIN_NUM_DELEGATES;
                ...
                info["name_size_max"] = GDW_BLOCKCHAIN_MAX_NAME_SIZE;
                info["memo_size_max"] = GDW_BLOCKCHAIN_MAX_MESSAGE_SIZE;
                info["data_size_max"] = GDW_BLOCKCHAIN_MAX_NAME_DATA_SIZE;

                info["symbol_size_max"] = GDW_BLOCKCHAIN_MAX_SGDW_SYMBOL_SIZE;
                info["symbol_size_min"] = GDW_BLOCKCHAIN_MIN_SYMBOL_SIZE;
                info["asset_shares_max"] = GDW_BLOCKCHAIN_MAX_SHARES;
                info["short_symbol_asset_reg_fee"] = _chain_db->get_asset_registration_fee(GDW_BLOCKCHAIN_MIN_SYMBOL_SIZE);
                info["long_symbol_asset_reg_fee"] = _chain_db->get_asset_registration_fee(GDW_BLOCKCHAIN_MAX_SGDW_SYMBOL_SIZE);

                info["statistics_enabled"] = _chain_db->get_statistics_enabled();

                info["relay_fee"] = _chain_db->get_relay_fee();
                info["max_pending_queue_size"] = GDW_BLOCKCHAIN_MAX_PENDING_QUEUE_SIZE;
                info["max_trx_per_second"] = GDW_BLOCKCHAIN_MAX_TRX_PER_SECOND;
                ...
                if (fp != NULL)
                {
                    while (true)
                    {
                        ...
                        if (fgets(c, 256, fp) != NULL)
                        {
                            ...
                            std::string allstring = (std::string)gdwcore::consensus::Address(gdwcore::consensus::PtsAddress(pow_address));
                            allstring += balance;
            ...
            gdwcore::consensus::BlockchainSecurityState ClientImpl::blockchain_get_security_state()const
            {
                // set limit in  simulator state
                if (_chain_db->get_is_in_simulator())
                    FC_THROW_EXCEPTION(simulator_command_forbidden, "in simulator, this command is forbidden, you cannot call it!");
                ...
                state.estimated_confirmation_seconds = (uint32_t)(required_confirmations * GDW_BLOCKCHAIN_BLOCK_INTERVAL_SEC);
                state.participation_rate = participation_rate;
                if (!blockchain_is_synced())
                {
                    state.alert_level = gdwcore::consensus::BlockchainSecurityState::grey;
                }
                else if (required_confirmations <= GDW_BLOCKCHAIN_NUM_DELEGATES / 2
                    && participation_rate > 80)
                {
                    state.alert_level = gdwcore::consensus::BlockchainSecurityState::green;
                }
                else if (required_confirmations > GDW_BLOCKCHAIN_NUM_DELEGATES
                    || participation_rate < 60)
                {
                    state.alert_level = gdwcore::consensus::BlockchainSecurityState::red;
                }
                else
                {
                    state.alert_level = gdwcore::consensus::BlockchainSecurityState::yellow;
                }
            ...
            bool ClientImpl::blockchain_is_synced() const
            {
                ...
                return (gdwcore::consensus::now() - _chain_db->get_head_block().timestamp) < fc::seconds(GDW_BLOCKCHAIN_NUM_DELEGATES * GDW_BLOCKCHAIN_BLOCK_INTERVAL_SEC);
            }
            ...
            std::string ClientImpl::blockchain_export_fork_graph(uint32_t start_block, uint32_t end_block, const gdwcore::consensus::FilePath& filename)const
            {
                // set limit in  simulator state
                if (_chain_db->get_is_in_simulator())
                    FC_THROW_EXCEPTION(simulator_command_forbidden, "in simulator, this command is forbidden, you cannot call it!");
            ...
            bool ClientImpl::blockchain_verify_signature(const string& signer, const fc::sha256& hash, const fc::ecc::compact_signature& signature) const
            {
                ...
                catch (...)
                {
                    try {
                        ...
                        // NB LHS of == operator is gdwcore::consensus::public_key_type and RHS is fc::ecc::public_key,
            ...
            gdwcore::consensus::Asset ClientImpl::blockchain_unclaimed_genesis() const
            {
                // set limit in  simulator state
                if (_chain_db->get_is_in_simulator())
                    FC_THROW_EXCEPTION(simulator_command_forbidden, "in simulator, this command is forbidden, you cannot call it!");
