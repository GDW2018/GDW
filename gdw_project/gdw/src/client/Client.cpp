...
//#include <gdwcore/mail/server.hpp>
...
namespace gdwcore {
    namespace client {
        ...
        const string GDW_MESSAGE_MAGIC = "Gdw Signed Message:\n";
        ...
        program_options::variables_map parse_option_variables(int argc, char** argv)
        {
            ...
            {
                std::cout << fc::json::to_pretty_string(gdwcore::client::version_info()) << "\n";
                exit(0);
            }
        ...
        fc::path get_data_dir(const program_options::variables_map& option_variables)
        {
            try {
                ...
                if (option_variables.count("data-dir"))
                {
					...
					if (gdwcore::utilities::isGBK(temp_data_dir.data()))
					{
						temp_data_dir = gdwcore::console::GBKToUTF8(temp_data_dir);
					}
...
#ifdef GDW_TEST_NETWORK
                        //dir_name += "-Test" + std::to_string(GDW_TEST_NETWORK_VERSION);
                        dir_name += "-Test";
#endif
                        return dir_name;
                    };

                    datadir = fc::app_path() / get_os_specific_dir_name(GDW_BLOCKCHAIN_NAME);
                }
                return datadir;
        ...
        void load_checkpoints(const fc::path& data_dir)
        {
            try {
                ...
                decltype(gdwcore::consensus::detail::CHECKPOINT_BLOCKS) external_checkpoints;
                ...
                {
                    decltype(gdwcore::consensus::detail::CHECKPOINT_BLOCKS) external_checkpoints;
                    if (gdwcore::consensus::detail::CHECKPOINT_BLOCKS.empty() || external_checkpoints.crbegin()->first >= gdwcore::consensus::detail::CHECKPOINT_BLOCKS.crbegin()->first)
                    {
                        ulog("Using blockchain checkpoints from file: ${x}", ("x", checkpoint_file.preferred_string()));
                        gdwcore::consensus::detail::CHECKPOINT_BLOCKS = external_checkpoints;
                        return;
                    }
                }
                ...
                if (!file_exception.valid())
                {
                    ...
                    fc::json::save_to_file(gdwcore::consensus::detail::CHECKPOINT_BLOCKS, checkpoint_file);
                }
                else
                {
                ...
                if (!gdwcore::consensus::detail::CHECKPOINT_BLOCKS.empty())
        ...
        namespace detail
        {
                ...
                const program_options::variables_map& option_variables)
            {
                if (option_variables.count("server") || option_variables.count("daemon") || cfg.rpc.enable)
                {
                    ...
                    // gdwcore::rpc::rpc_server::config rpc_config(cfg.rpc);
            ...
            void ClientImpl::start_delegate_loop()
            {
                ...
                if (!_time_discontinuity_connection.connected())
                    _time_discontinuity_connection = gdwcore::consensus::time_discontinuity_signal.connect([=](){ reschedule_delegate_loop(); });
            ...
            void ClientImpl::delegate_loop()
            {
                ...
                {
                    set_target_connections(GDW_NET_DELEGATE_DESIRED_CONNECTIONS);
                ...
                if (next_block_time.valid())
                {
                    ...
                    if (*next_block_time <= now)
                    {
                        try
                        {
                            ...
                            FC_ASSERT((now - *next_block_time) < fc::seconds(GDW_BLOCKCHAIN_BLOCK_INTERVAL_SEC),
                ...
                uint32_t slot_number = gdwcore::consensus::get_slot_number(now);
                time_point_sec next_slot_time = gdwcore::consensus::get_slot_start_time(slot_number + 1);
                ilog("Rescheduling delegate loop for time: ${t}", ("t", next_slot_time));

                time_point scheduled_time = next_slot_time;
                if (gdwcore::consensus::ntp_time().valid())
                    scheduled_time -= gdwcore::consensus::ntp_error();
            ...
            void ClientImpl::start_rebroadcast_pending_loop()
            {
                if (!_rebroadcast_pending_loop_done.valid() || _rebroadcast_pending_loop_done.ready())
                    ...
                    fc::time_point::now() + fc::seconds((int64_t)(GDW_BLOCKCHAIN_BLOCK_INTERVAL_SEC * 1.3)),
                    "rebroadcast_pending");
            }
            void ClientImpl::start_fork_update_time_loop()
            {
                if (!_fork_update_loop_done.valid() || _fork_update_loop_done.ready())
                    ...
                    fc::time_point::now() + fc::seconds((int64_t)(GDW_BLOCKCHAIN_BLOCK_INTERVAL_SEC * 6)),
                    "rebroadcast_pending");
            }
            void ClientImpl::fork_update_time_loop()
            {
                ...
                {
                    gdwcore::consensus::update_ntp_time();
                    _chain_db->set_forkdb_num(fork_num);
                }
            }
            gdwcore::consensus::PublicKeyType ClientImpl::wallet_get_account_owner_publickey(const std::string& account_name)
            {
                // set limit in  simulator state
                if (_chain_db->get_is_in_simulator())
                    FC_THROW_EXCEPTION(simulator_command_forbidden, "in simulator, this command is forbidden, you cannot call it!");
            ...
            void ClientImpl::rebroadcast_pending_loop()
            {
                ...
                else
                {
                    try
                    {
                        ...
                        std::vector<gdwcore::consensus::SignedTransaction> temp_vec;
                    ...
                    try
                    {
                        ...
                        if (fork_data && fork_data->is_known)
                        {
                            if (sync_mode && !fork_data->is_linked)
                                FC_THROW_EXCEPTION(gdwcore::consensus::unlinkable_block,
                        ...
                        else
                        {
                            ...
                            if (sync_mode && !result.is_linked)
                                FC_THROW_EXCEPTION(gdwcore::consensus::unlinkable_block, "The blockchain accepted this block, but it isn't linked");
            ...
            bool ClientImpl::has_item(const gdwcore::network::ItemId& id)
            {
            ...
            */
            bool ClientImpl::handle_message(const gdwcore::network::Message& message_to_handle, bool sync_mode)
            {
                if (g_client_quit)
                    return true;
                ...
                try
                {
                    ...
                    case block_message_type:
                    {
                        ...
                        if (block_message_to_handle.block.block_size() > GDW_BLOCKCHAIN_MAX_BLOCK_SIZE)
                        {
                        ...
                        gdwcore::consensus::BlockIdType old_head_block = _chain_db->get_head_block_id();
                    ...
                    case trx_message_type:
                    {
                        ...
                        if (trx_message_to_handle.trx.data_size() > GDW_BLOCKCHAIN_MAX_TRX_SIZE)
                        {
                ...
                catch (const gdwcore::consensus::insufficient_relay_fee& original_exception)
                {
                    FC_THROW_EXCEPTION(gdwcore::network::insufficient_relay_fee, "Insufficient relay fee; do not propagate!",
                        ("original_exception", original_exception.to_detail_string()));
                }
                catch (const gdwcore::consensus::block_older_than_undo_history& original_exception)
                {
                    FC_THROW_EXCEPTION(gdwcore::network::block_older_than_undo_history, "Block is older than undo history, stop fetching blocks!",
                        ("original_exception", original_exception.to_detail_string()));
                }
            }
            ...
            */
            std::vector<gdwcore::network::ItemHashType> ClientImpl::get_item_ids(uint32_t item_type,
                const std::vector<gdwcore::network::ItemHashType>& blockchain_synopsis,
                uint32_t& remaining_item_count,
                uint32_t limit /* = 2000 */)
            {
                // limit = 20; // for testing
                FC_ASSERT(item_type == gdwcore::client::block_message_type);
                ...
                gdwcore::network::ItemHashType last_seen_block_hash;
                for (const gdwcore::network::ItemHashType& item_hash : boost::adaptors::reverse(blockchain_synopsis))
                {
                    try
                    {
                ...
                std::vector<gdwcore::network::ItemHashType> hashes_to_return;
            ...
            std::vector<gdwcore::network::ItemHashType> ClientImpl::get_blockchain_synopsis(uint32_t item_type,
            ...
            {
                FC_ASSERT(item_type == gdwcore::client::block_message_type);
                std::vector<gdwcore::network::ItemHashType> synopsis;
                uint32_t high_block_num = 0;
                uint32_t non_fork_high_block_num = 0;
                std::vector<BlockIdType> fork_history;

                if (reference_point != gdwcore::network::ItemHashType())
                {
            ...
            gdwcore::network::Message ClientImpl::get_item(const gdwcore::network::ItemId& id)
            {
                if (id.item_type == block_message_type)
                {
                    //   uint32_t block_number = _chain_db->get_block_num(id.item_hash);
                    gdwcore::client::BlockMessage block_message_to_send(_chain_db->get_block(id.item_hash));
            ...
            fc::path ClientImpl::script_compile(const fc::path& filename) const
            {
                // set limit in  simulator state
                if (_chain_db->get_is_in_simulator())
                    FC_THROW_EXCEPTION(simulator_command_forbidden, "in simulator, this command is forbidden, you cannot call it!");
                ...
                try{
                    ...
                    {
                        FC_THROW_EXCEPTION(gdwcore::consensus::invalid_script_source_filename, "script source file name should end with .lua or .glua");
                    }
					auto p_lua_module = std::make_shared<UvmModuleByteStream>();
                    FC_ASSERT(p_lua_module, "Alloc memory for UvmModuleByteStream failed!");
					::gdwcore::consensus::contract_engine::ContractEngineBuilder builder;
					...
					{
						FC_THROW_EXCEPTION(script_compile_fail, e.what());
					}
                    if (save_code_to_file(out_filename, p_lua_module.get(), err_msg) < 0)
                    {
                        FC_THROW_EXCEPTION(gdwcore::consensus::save_bytecode_to_scriptfile_fail, err_msg);
                    }
            ...
            uint32_t ClientImpl::get_block_number(const gdwcore::network::ItemHashType& block_id)
            {
                return _chain_db->get_block_num(block_id);
            }

            fc::time_point_sec ClientImpl::get_block_time(const gdwcore::network::ItemHashType& block_id)
            {
                if (block_id == gdwcore::network::ItemHashType())
                {
            ...
            fc::time_point_sec ClientImpl::get_blockchain_now()
            {
                ...
                return gdwcore::consensus::nonblocking_now();
            }

            gdwcore::network::ItemHashType ClientImpl::get_head_block_id() const
            {
            ...
            {
                return gdwcore::consensus::estimate_last_known_fork_from_git_revision_timestamp(unix_timestamp);
            }
            ...
            void ClientImpl::blocks_too_old_monitor_task()
            {
                ...
                if (_chain_db->get_head_block().timestamp < gdwcore::consensus::now() - fc::seconds(GDW_BLOCKCHAIN_BLOCK_INTERVAL_SEC * 2) &&
                    !_sync_mode &&
        ...
        Client::Client(const std::string& user_agent,
            gdwcore::network::SimulatedNetworkPtr network_to_connect_to)
            : my(new detail::ClientImpl(this, user_agent))
        {
            ...
            const std::function<void(float)> replay_status_callback)
        {
            try {
                ...
                my->_wallet = std::make_shared<gdwcore::wallet::Wallet>(my->_chain_db, my->_config.wallet_enabled);
                my->_wallet->set_data_directory(wallet_dir / "wallets");

                //delete?
                //if (my->_config.mail_server_enabled)
                //{
                //my->_mail_server = std::make_shared<gdwcore::mail::server>();
                //my->_mail_server->open(data_dir / "mail");
                //}
                //my->_mail_client = std::make_shared<gdwcore::mail::client>(my->_wallet, my->_chain_db);
                ...
                if (!my->_p2p_node)
                    my->_p2p_node = std::make_shared<gdwcore::network::Node>(my->_user_agent);
        ...
        gdwcore::rpc::RpcServerPtr Client::get_rpc_server()const { return my->_rpc_server; }
        gdwcore::network::NodePtr Client::get_node()const { return my->_p2p_node; }

        fc::variant_object version_info()
        {
            string client_version(gdwcore::utilities::git_revision_description);
            const size_t pos = client_version.find('/');
            if (pos != string::npos && client_version.size() > pos)
                client_version = client_version.substr(pos + 1);
#ifdef GDW_TEST_NETWORK
            //client_version += "-testnet-" + std::to_string(GDW_TEST_NETWORK_VERSION);
...
#if defined(GDW_CLIENT_JENKINS_BUILD_NUMBER)
            info["jenkins_build_number"] = GDW_CLIENT_JENKINS_BUILD_NUMBER;
#endif
#if defined(GDW_CLIENT_JENKINS_BUILD_URL)
            info["jenkins_build_url"] = GDW_CLIENT_JENKINS_BUILD_URL;
#endif
        ...
        void Client::start_networking(std::function<void()> network_started_callback)
        {
            ...
            {
                gdwcore::network::ChainDownloader* chain_downloader = new gdwcore::network::ChainDownloader();
        ...
        void Client::configure_from_command_line(int argc, char** argv)
        {
            ...
            {
                my->_cli = new gdwcore::console::Cli(this, nullptr, &std::cout);
                return;
            }
            g_client = this;
            // parse command-line options
            ...
            fc::path datadir = gdwcore::client::get_data_dir(option_variables);
            fc::path walletDir = gdwcore::client::get_wallet_dir(option_variables, datadir);
            ...
            // else we use the default set in gdwcore::network::node
            ...
            else //we will accept input from the console
            {
                ...
                if (option_variables.count("log-commands") <= 0)
                {
                    ...
                    my->_cli = new gdwcore::console::Cli(this, my->_command_script_holder.get(), &std::cout);
                }
                else
                {
                    ...
                    my->_cli = new gdwcore::console::Cli(this, my->_command_script_holder.get(), my->_tee_stream.get());
            ...
            if (option_variables["upnp"].as<bool>())
            {
                ...
                my->_upnp_service = std::unique_ptr<gdwcore::network::UpnpService>(new gdwcore::network::UpnpService);
			...
			*/
            ...
            start_networking([=]{
                ...
                else if (!option_variables.count("disable-default-peers"))
                {
                    for (string default_peer : my->_config.default_peers)
                    {
                        ...
                        catch (gdwcore::network::endpoint_in_blacklist)
                        {
                        }
                    }
                }
            ...
            {
                my->_chain_server = std::unique_ptr<gdwcore::network::ChainServer>(
                    new gdwcore::network::ChainServer(my->_chain_db,
        ...
        gdwcore::network::NodeIdType Client::get_node_id() const
        {
        ...
        void Client::add_node(const string& remote_endpoint, int32_t oper_flag)
        {
            ...
            catch (const gdwcore::network::already_connected_to_requested_peer&)
            {
            }
        }
        ...
        {
            gdwcore::network::ItemId head_item_id;
            head_item_id.item_type = gdwcore::client::block_message_type;
            uint32_t last_block_num = my->_chain_db->get_head_block_num();
            if (last_block_num == (uint32_t)-1)
                head_item_id.item_hash = gdwcore::network::ItemHashType();
            else
                head_item_id.item_hash = my->_chain_db->get_head_block_id();
            my->_p2p_node->sync_from(head_item_id, gdwcore::consensus::get_list_of_fork_block_numbers());
        ...
        bool Client::handle_message(const gdwcore::network::Message& message, bool sync_mode)
        {
        ...
        */
        ...
        gdwcore::api::CommonApi* Client::get_impl() const
        {
            return my.get();
        }
