...
namespace gdwcore {
    namespace wallet {
        ...
        namespace detail {
            ...
            void WalletImpl::state_changed(const PendingChainStatePtr& state)
            {
                ...
                if (_blockchain->get_head_block_num() < last_unlocked_scanned_number)
                {
                    ...
                    self->set_last_scanned_block_number_for_ub(_blockchain->get_head_block_num());
                }
            }
            ...
            {
                if (!self->is_open() || !self->is_unlocked()) return;
                auto ntptime = gdwcore::consensus::ntp_time();
				fc::time_point t = (ntptime.valid() ? *ntp_time() : fc::time_point::now());
                if ((t - _blockchain->get_head_block_timestamp()) <= fc::seconds(GDW_BLOCKCHAIN_BLOCK_INTERVAL_SEC * 2))
            ...
            void WalletImpl::handle_events(const vector<EventOperation>& event_vector)
            {
                ...
                for (auto i : event_vector)
                {
                    ...
                    for (auto sid : res->second)
                    {
                        ...
                        auto code_stream = uvm::lua::api::global_uvm_chain_api->get_bytestream_from_code(scope.L(), script->code);
                        if (!code_stream)
                            continue;
                        try{
                            uvm::lua::lib::add_global_bool_variable(scope.L(), "truncated", i.is_truncated);
							uvm::lua::lib::add_global_string_variable(scope.L(), "event_type", i.event_type.c_str());
							uvm::lua::lib::add_global_string_variable(scope.L(), "param", i.event_param.c_str());
							uvm::lua::lib::add_global_string_variable(scope.L(), "contract_id", i.id.AddressToString().c_str());
							uvm::lua::lib::run_compiled_bytestream(scope.L(), code_stream.get());
                        }
            ...
            */
                ...
                try {
                    ...
                    try
                    {
                        ...
                        const auto update_progress = [=](const uint32_t count)
                        {
                            ...
                            if (count % 10000 == 0) ulog("Scanning ${p} done...", ("p", gdwcore::console::pretty_percent(_scan_progress, 1)));
                        };
                        ...
                        while (current_block_num <= head_block_num && count < limit && !_scan_in_progress.canceled())
                        {
                            ...
                            self->set_last_scanned_block_number_for_ub(prev_block_num);
                            ++current_block_num;
            ...
            void WalletImpl::upgrade_version()
            {
                ...
                if (current_version > GDW_WALLET_VERSION)
                {
                    FC_THROW_EXCEPTION(unsupported_version, "Wallet version newer than client supports!",
                        ("wallet_version", current_version)("supported_version", GDW_WALLET_VERSION));
                }
                else if (current_version == GDW_WALLET_VERSION)
                {
                    return;
                }
                ...
                try
                {
                    ...
                    if (current_version < 100)
                    {
                        ...
                        _blockchain->scan_balances([&](const BalanceEntry& bal_rec)
                        {
                            ...
                            const auto id = bal_rec.id();
							present |= _wallet_db.lookup_transaction(fc::ripemd160(id.operator string())).valid();
                        });
                        ...
                        if (present)
                        {
                            const function<void(void)> rescan = [&]()
                            {
                                ...
                                _blockchain->scan_balances([&](const BalanceEntry& bal_rec)
                                {
                                    if (!bal_rec.snapshot_info.valid()) return;
                                    const auto id = bal_rec.id();
									_wallet_db.remove_transaction(fc::ripemd160(id.operator string()));
                                });
                                scan_balances();
                            };
                    ...
                    {
                        self->set_transaction_fee(Asset(GDW_WALLET_DEFAULT_TRANSACTION_FEE));
                        self->set_transaction_imessage_fee_coe(GDW_BLOCKCHAIN_MIN_MESSAGE_FEE_COE);
                        self->set_transaction_imessage_soft_max_length(GDW_BLOCKCHAIN_MAX_SOFT_MAX_MESSAGE_SIZE);
                    }

                    if (current_version < 106)
                    {
                        self->set_transaction_expiration(GDW_WALLET_DEFAULT_TRANSACTION_EXPIRATION_SEC);
                    }
                    ...
                    {
                        self->set_last_scanned_block_number_for_ub(0);
                    }
                    if (_unlocked_upgrade_tasks.empty())
                    {
                        self->set_version(GDW_WALLET_VERSION);
                ...
                const string& brainkey)
            {
                try {
                    ...
                    if (password.size() < GDW_WALLET_MIN_PASSWORD_LENGTH)
             ...
             *  returned will contain no more than GDW_BLOCKCHAIN_MAX_SLATE_SIZE delegates.
             */
            SlateEntry WalletImpl::get_delegate_slate(const VoteStrategy strategy)const
            {
                ...
                {
                    slate_size = std::min<size_t>(GDW_BLOCKCHAIN_MAX_SLATE_SIZE, for_candidates.size());
                }
                else if (strategy == vote_random)
                {
                    slate_size = std::min<size_t>(GDW_BLOCKCHAIN_MAX_SLATE_SIZE / 3, for_candidates.size());
                    slate_size = rand() % (slate_size + 1);
                }
                else if (strategy == vote_recommended && for_candidates.size() < GDW_BLOCKCHAIN_MAX_SLATE_SIZE)
                {
                    ...
                    while (for_candidates.size() < GDW_BLOCKCHAIN_MAX_SLATE_SIZE && recommended_candidate_ranks.size() > 0)
                    {
                        int best_rank = 0;
                ...
                FC_ASSERT(entry.slate.size() <= GDW_BLOCKCHAIN_MAX_SLATE_SIZE);
            ...
            const string& brainkey)
        {
            try {
                ...
                try
                {
                    ...
                    open(wallet_name);
                    unlock(password, GDW_WALLET_DEFAULT_UNLOCK_TIME_SEC);
                }
                catch (...)
                {
        ...
        gdwcore::wallet::WalletTransactionEntry Wallet::transfer_asset_to_contract_without_signature(
            ...
            bool is_testing)
        {
            try {
                ...
                FC_ASSERT(is_open(), "Wallet not open!");
                FC_ASSERT(amount_to_transfer_symbol == GDW_BLOCKCHAIN_SYMBOL, "Asset symbol should be HSR");
                FC_ASSERT(data_ptr->is_valid_symbol(amount_to_transfer_symbol), "Invalid asset symbol");

                PublicKeyType  sender_public_key(from_account_public_key_str);
                ...
                FC_ASSERT(asset_id == 0, "Asset symbol should be GDW");
                ...
                Asset asset_to_transfer = my->_blockchain->to_ugly_asset(real_amount_to_transfer, GDW_BLOCKCHAIN_SYMBOL);
        ...
        void Wallet::create_from_json(const path& filename, const string& wallet_name, const string& passphrase)
        {
            try {
                ...
                try
                {
                    ...
                    close();
                    open(wallet_name);
                    unlock(passphrase, GDW_WALLET_DEFAULT_UNLOCK_TIME_SEC);
                }
                catch (...)
                {
        ...
        void Wallet::unlock(const string& password, uint32_t timeout_seconds)
        {
            try {
                ...
                try
                {
                    ...
                    const uint32_t max_timeout_seconds = std::numeric_limits<uint32_t>::max() - gdwcore::consensus::now().sec_since_epoch();
        ...
        void Wallet::lock()
        {
            ...
            ilog("Wallet locked at time: ${t}", ("t", gdwcore::consensus::now()));

			if (my->_blockchain->get_is_in_simulator())
			{
				my->_blockchain->set_is_in_simulator(false);
				my->_blockchain->clear_simulator_pending_state();
			}
        }
        ...
        bool Wallet::check_passphrase(const string& passphrase)
        {
            try {
                ...
                if (passphrase.size() < GDW_WALLET_MIN_PASSWORD_LENGTH) FC_CAPTURE_AND_THROW(password_too_short);
        ...
        void Wallet::change_passphrase(const string & old_passphrase, const string& new_passphrase)
        {
            try {
                if (NOT is_open()) FC_CAPTURE_AND_THROW(wallet_closed);
                if (NOT is_unlocked()) FC_CAPTURE_AND_THROW(wallet_locked);
                if (new_passphrase.size() < GDW_WALLET_MIN_PASSWORD_LENGTH) FC_CAPTURE_AND_THROW(password_too_short);
            ...
            const variant& private_data)
        {
            try {
                ...
                if (num_accounts_before == 0)
                {
                    ...
                    set_last_scanned_block_number_for_ub(0);
                }
            ...
            const variant& private_data)
        {
            try {
                ...
                if (current_account.valid())
                {
                    ...
                    {
                        gdwcore::consensus::AccountEntry& as_blockchain_account_entry = *current_account;
                ...
                else
                {
                    ...
                    if (!current_registered_account.valid())
                    {
                        const time_point_sec now = gdwcore::consensus::now();
        ...
        void Wallet::accountsplit(const string & original, string & to_account, string & sub_account)
        {
            ...
            if (INVALIDE_SGDW_ADDRESS == sub_account)
            {
                sub_account = "";
            }
            else
            ...
            bool create_account)
        {
            try {
                ...
                if (blockchain_account_entry.valid())
                {
                    ...
                    account_entry->set_active_key(gdwcore::consensus::now(), active_public_key);
                    account_entry->last_update = gdwcore::consensus::now();
            ...
            bool create_account)
        {
            try {
                ...
                auto key = gdwcore::utilities::wif_to_key(wif_key);
                if (key.valid())
                {
        ...
        VoteSummary Wallet::get_vote_status(const string& account_name)
        {
            ...
            for (const auto& item : items)
            {
                ...
                for (const auto& entry : entrys)
                {
                    if (entry.asset_id() != 0)
                        continue;
                    if (entry.slate_id() != my_slate.id() && entry.balance > 1 * GDW_BLOCKCHAIN_PRECISION) // ignore dust
                    ...
                    total_possible += entry.get_spendable_balance(my->_blockchain->now()).amount * GDW_BLOCKCHAIN_MAX_SLATE_SIZE;
                }
            }
            ...
            bool sign)
        {
            try {
                ...
                trx.expiration = gdwcore::consensus::now() + get_transaction_expiration();
            ...
            bool sign)
        {
            try {
                ...
                const auto version = gdwcore::client::version_info()["client_version"].as_string();
        ...
        void Wallet::repair_entrys(const optional<string>& collecting_account_name)
        {
            try {
                ...
                for (const auto& item : items)
                {
                    ...
                    if (name.find(GDW_ADDRESS_PREFIX) != 0)
                        continue;
        ...
        std::map<std::string, gdwcore::consensus::DelegatePaySalary> Wallet::query_delegate_salarys()
        {
            FC_ASSERT(is_open());
            FC_ASSERT(is_unlocked());
            std::map<std::string, gdwcore::consensus::DelegatePaySalary> Salary_Map;
        ...
        {
            if (my->_blockchain->get_is_in_simulator())
                return my->_blockchain->get_simulator_pending_state();

            return my->_blockchain;

        }

        WalletTransactionEntry Wallet::contract_register(const string& owner, const fc::path codefile, const string& asset_symbol, double init_limit, bool is_testing)
        {
            ChainInterfacePtr data_ptr = get_correct_state_ptr();
            ...
            {
                FC_THROW_EXCEPTION(gdwcore::consensus::invalid_contract_filename, "contract bytecode file name should end with .gpc");
            }
            ...
            FC_ASSERT(asset_symbol == GDW_BLOCKCHAIN_SYMBOL, "asset_symbol must be GDW");
			...
			uvm::blockchain::Code contract_code(codefile);

            PublicKeyType  owner_public_key = get_owner_public_key(owner);
            Address        owner_address = Address(owner_public_key);

			auto account_info = get_account_for_address(owner_address);
			FC_ASSERT(account_info.valid(), "Can not get owner address from wallet");
			/*bool is_delegate = account_info->is_delegate();
			if (NOT is_delegate)
			{
				std::string bytecode_hash = contract_code.code_hash;
				auto tpl_entry = data_ptr->get_contract_template_entry(bytecode_hash);
				FC_ASSERT(tpl_entry.valid(), "unsupported contract bytecode");
			}*/
			...
			{
				if (my->_blockchain->get_is_in_simulator())
					simulator_get_enough_balances(owner, asset_limit + register_fee + fee + margin, balances, required_signatures);
				else
            ...
            ContractIdType contract_id = trx.contract_register(contract_code, owner_public_key, asset_limit,fee, balances);//插入合约注册op
            FC_ASSERT(register_fee.asset_id == 0, "register fee must be GDW");
            FC_ASSERT(margin.asset_id == 0, "register fee must be GDW");
        ...
        std::vector<gdwcore::consensus::Asset> Wallet::contract_register_testing(const string& owner, const fc::path codefile)
        {
            FC_ASSERT(is_open(), "Wallet not open!");
            FC_ASSERT(is_unlocked(), "Wallet not unlock!");
            FC_ASSERT(my->is_receive_account(owner), "Invalid account name");
            ...
            auto trans_entry = contract_register(owner,
                codefile,
                GDW_BLOCKCHAIN_SYMBOL,
                (double)asset_for_exec.amount / GDW_BLOCKCHAIN_PRECISION, 
                true
                );
            ...
            std::vector<gdwcore::consensus::Asset> asset_vec;
		...
		gdwcore::wallet::WalletTransactionEntry Wallet::contract_call(const string caller, const ContractIdType contract, const string method, const string& arguments, const string& asset_symbol, double cost_limit, bool is_testing)
		{
			ChainInterfacePtr data_ptr = get_correct_state_ptr();
			...
			{
				if (my->_blockchain->get_is_in_simulator())
					simulator_get_enough_balances(caller, asset_limit + fee, balances, required_signatures);
				else
        ...
        std::vector<gdwcore::consensus::Asset> Wallet::contract_call_testing(const string caller, const ContractIdType contract, const string method, const string& arguments)
        {
            FC_ASSERT(is_open(), "Wallet not open!");
            FC_ASSERT(is_unlocked(), "Wallet not unlock!");
            FC_ASSERT(my->is_receive_account(caller), "Invalid account name");
            ...
            auto trans_entry = contract_call(caller,
                contract,
                method,
                arguments,
                GDW_BLOCKCHAIN_SYMBOL,
        ...
        std::string Wallet::contract_call_offline(const string caller, const ContractIdType contract, const string method, const string& arguments)
        {
            FC_ASSERT(is_open(), "Wallet not open!");
            FC_ASSERT(is_unlocked(), "Wallet not unlock!");
            FC_ASSERT(my->is_receive_account(caller), "Invalid account name");
			...
			uvm::lua::lib::UvmStateScope scope;
			uvm::lua::lib::add_global_string_variable(scope.L(), "caller", (((string)(caller_public_key)).c_str()));
			uvm::lua::lib::add_global_string_variable(scope.L(), "caller_address", ((string)(Address(caller_address))).c_str());
			uvm::lua::lib::set_lua_state_value(scope.L(), "evaluate_state", statevalue, UvmStateValueType::LUA_STATE_VALUE_POINTER);

			uvm::lua::api::global_uvm_chain_api->clear_exceptions(scope.L());
            ...
            int exception_code = uvm::lua::lib::get_lua_state_value(scope.L(), "exception_code").int_value;
            char* exception_msg = (char*)uvm::lua::lib::get_lua_state_value(scope.L(), "exception_msg").string_value;
            if (exception_code > 0)
            {
                gdwcore::consensus::contract_error con_err(32000, "exception", exception_msg);
                throw con_err;
            }
		...
		AccountBalanceEntrySummaryType Wallet::simulator_get_spendable_account_balance_entries(const string& account_name)
		{
			try
			{
				// set the balances state for simulator
				PendingChainStatePtr simulator_state = my->_blockchain->get_simulator_pending_state();
		...
		void Wallet::simulator_get_enough_balances(const string& account_name, const Asset target, std::map<BalanceIdType, ShareType>& balances, unordered_set<Address>& required_signatures)
		{
			try {
				// set the balances state for simulator
				PendingChainStatePtr simulator_state = my->_blockchain->get_simulator_pending_state();

				const AccountBalanceEntrySummaryType balance_entrys = simulator_get_spendable_account_balance_entries(account_name);
				...
				for (const auto& entry : balance_entrys.at(account_name))
				{
					const Asset balance = entry.get_spendable_balance(simulator_state->now());
					if (balance.amount <= 0 || balance.asset_id != amount_remaining.asset_id)
						continue;
        ...
        vector<ScriptEntry> Wallet::scripts_list()
        {
            try{
                FC_ASSERT(is_open(), "Wallet not open!");
                FC_ASSERT(is_unlocked(), "Wallet not unlock!");
        ...
        gdwcore::wallet::oScriptEntry Wallet::get_script_entry(const ScriptIdType& script_id)
        {
            try{
                FC_ASSERT(is_open(), "Wallet not open!");
                FC_ASSERT(is_unlocked(), "Wallet not unlock!");
        ...
        gdwcore::consensus::ScriptIdType Wallet::script_add(const fc::path& filename, const string& description/*=string("")*/)
        {
            try{
                FC_ASSERT(is_open(), "Wallet not open!");
                FC_ASSERT(is_unlocked(), "Wallet not unlock!");
        ...
        void Wallet::script_disable(const ScriptIdType& script_id)
        {
            try{
                FC_ASSERT(is_open(), "Wallet not open!");
                FC_ASSERT(is_unlocked(), "Wallet not unlock!");
        ...
        void Wallet::script_enable(const ScriptIdType& script_id)
        {
            try{
                FC_ASSERT(is_open(), "Wallet not open!");
                FC_ASSERT(is_unlocked(), "Wallet not unlock!");
        ...
        vector<ScriptIdType> Wallet::script_list_event_handler(const ContractIdType& contract_id, const std::string& event_type)
        {
            try{
                FC_ASSERT(is_open(), "Wallet not open!");
                FC_ASSERT(is_unlocked(), "Wallet not unlock!");
        ...
        void Wallet::script_add_event_handler(const ContractIdType& contract_id, const std::string& event_type, const ScriptIdType& script_id, uint32_t index)
        {
            try{
                FC_ASSERT(is_open(), "Wallet not open!");
                FC_ASSERT(is_unlocked(), "Wallet not unlock!");
        ...
        void Wallet::script_delete_event_handler(const ContractIdType& contract_id, const std::string& event_type, const ScriptIdType& script_id)
        {
            try{
                FC_ASSERT(is_open(), "Wallet not open!");
                FC_ASSERT(is_unlocked(), "Wallet not unlock!");
		...
		std::vector<std::string> Wallet::script_get_events_bound(const std::string& script_id)
		{
			FC_ASSERT(is_open(), "Wallet not open!");
			FC_ASSERT(is_unlocked(), "Wallet not unlock!");
        ...
        gdwcore::wallet::WalletTransactionEntry Wallet::transfer_asset_to_contract(
            const string& real_amount_to_transfer,
            const string& amount_to_transfer_symbol,
            const string& from_account_name,
            const Address& to_contract_address,
            ...
            bool is_testing)
        {
            try {
                ...
                FC_ASSERT(is_open(), "Wallet not open!");
                FC_ASSERT(is_unlocked(), "Wallet not unlock!");
                FC_ASSERT(amount_to_transfer_symbol == GDW_BLOCKCHAIN_SYMBOL, "Asset symbol should be GDW");
                FC_ASSERT(data_ptr->is_valid_symbol(amount_to_transfer_symbol), "Invalid asset symbol");
                FC_ASSERT(my->is_receive_account(from_account_name), "Invalid account name");
				...
				{
					if (my->_blockchain->get_is_in_simulator())
						simulator_get_enough_balances(from_account_name, required_fees + asset_to_transfer + asset_for_exec, balances, required_signatures);
					else
        ...
        std::vector<gdwcore::consensus::Asset> Wallet::transfer_asset_to_contract_testing(
            const string& real_amount_to_transfer,
            const string& amount_to_transfer_symbol,
            const string& from_account_name,
            const Address& to_contract_address,
		...
		std::vector<gdwcore::consensus::Asset> Wallet::contract_upgrade_testing(
			...
			)
		{
			try
			{
				...
				if (margin_balance_entry->balance != GDW_DEFAULT_CONTRACT_MARGIN)
					FC_CAPTURE_AND_THROW(invalid_margin_amount, (margin_balance_entry->balance));


				Asset margin = Asset(GDW_DEFAULT_CONTRACT_MARGIN);
				Asset fee = get_transaction_fee(0);
				Asset asset_for_exec = my->_blockchain->get_amount(CONTRACT_TESTING_LIMIT_MAX);



				auto trans_entry = contract_upgrade(contract_id,
					upgrader_name,
					new_contract_name,
					new_contract_desc,
					GDW_BLOCKCHAIN_SYMBOL,
        ...
        WalletTransactionEntry Wallet::contract_upgrade(
            const Address& contract_id,
            const string& upgrader_name,
            const string& new_contract_name,
            const string& new_contract_desc,
			...
			bool is_testing)
        {
            try
			{
				...
				{
					if (my->_blockchain->get_is_in_simulator())
						simulator_get_enough_balances(upgrader_name, required_fees + asset_limit, balances, required_signatures);
					else
                ...
                trx.contract_upgrade(contract_id, new_contract_name, new_contract_desc, asset_limit, required_fees, balances);
				...
				FC_ASSERT(balance_entry->asset_id() == 0, "margin balance asset is not GDW!");
		...
		std::vector<gdwcore::consensus::Asset> Wallet::contract_destroy_testing(
			const Address& contract_id,
			...
			)
		{
			try
			{
				...
				auto trans_entry = contract_destroy(contract_id,
					destroyer_name,
					GDW_BLOCKCHAIN_SYMBOL,
					(double)asset_for_exec.amount / GDW_BLOCKCHAIN_PRECISION,
					sign,
        ...
        WalletTransactionEntry Wallet::contract_destroy(
            const Address& contract_id,
            const string& destroyer_name,
			const std::string& asset_symbol,
			double exec_limit,
			...
			bool is_testing)
        {
            try
			{
				...
				{
					if (my->_blockchain->get_is_in_simulator())
						simulator_get_enough_balances(destroyer_name, required_fees + asset_limit, balances, required_signatures);
					else
                ...
                trx.contract_destroy(contract_id, asset_limit, required_fees, balances);
                ...
                FC_ASSERT(balance_entry.valid(), "Can not get margin balance!");
                FC_ASSERT(balance_entry->asset_id() == 0, "margin balance asset is not GDW!");
                FC_ASSERT(balance_entry->balance == GDW_DEFAULT_CONTRACT_MARGIN, "invalid margin balance amount");
                ...
                {
                    FC_ASSERT(balance_entry->asset_id() == 0, "contract balance asset is not GDW!");
                }
            ...
            bool sign)
        {
            try {
                ...
                {
                    required_fees += Asset(GDW_BLOCKCHAIN_REGISTER_ACCOUNT_FEE, 0);
                }
        ...
        {
            try {
                FC_ASSERT(gdwcore::consensus::is_power_of_ten(precision));
                FC_ASSERT(is_open(), "Wallet not open!");
                FC_ASSERT(is_unlocked(), "Wallet not unlock!");
                ...
                FC_ASSERT(GDW_BLOCKCHAIN_MAX_SHARES > max_share_supply_in_internal_units);
        ...
        void Wallet::set_transaction_fee(const Asset& fee)
        {
            try {
                ...
                if (fee.amount < GDW_DEFAULT_TRANSACTION_FEE || fee.asset_id != 0)
        ...
        Asset Wallet::get_transaction_imessage_fee(const std::string & imessage)const
        {
            try
            {
                ...
                Asset require_fee;
                    ...
                    if (GDW_BLOCKCHAIN_MAX_FREE_MESSAGE_SIZE >= imessage.size())
                    {
                        return require_fee;
                    }
                    auto min_fee_coe = get_transaction_imessage_fee_coe();
                    require_fee.amount = min_fee_coe * (imessage.size() - GDW_BLOCKCHAIN_MAX_FREE_MESSAGE_SIZE);
        ...
        Asset Wallet::get_transaction_fee(const AssetIdType desired_fee_asset_id)const
        {
            try {
                ...
                Asset xts_fee(GDW_WALLET_DEFAULT_TRANSACTION_FEE, 0);
                try
                {
        ...
        void Wallet::set_last_scanned_block_number_for_ub(uint32_t block_num)
        {
            try {
                FC_ASSERT(is_open(), "Wallet not open!");
                my->_wallet_db.set_property(last_scanned_block_number_for_gdwcore, fc::variant(block_num));
            } FC_CAPTURE_AND_RETHROW()
        }
        ...
        uint32_t Wallet::get_last_scanned_block_number_for_ub()const
        {
            try {
                FC_ASSERT(is_open(), "Wallet not open!");
                try
                {
                    return my->_wallet_db.get_property(last_scanned_block_number_for_gdwcore).as<uint32_t>();
                }
                catch (...)
                {
                }
        ...
        void Wallet::set_transaction_imessage_fee_coe(const ImessageIdType& coe)
        {
            try
            {
                FC_ASSERT(is_open(), "Wallet not open!");
                    ...
                    if (coe < GDW_BLOCKCHAIN_MIN_MESSAGE_FEE_COE)
                    {
        ...
        void Wallet::set_transaction_imessage_soft_max_length(const ImessageLengthIdType& length)
        {
            try
            {
                FC_ASSERT(is_open(), "Wallet not open!");
                    ...
                    if (length < 0 || length > GDW_BLOCKCHAIN_MAX_MESSAGE_SIZE)
                    {
        ...
        void Wallet::set_transaction_expiration(uint32_t secs)
        {
            try {
                ...
                if (secs > GDW_BLOCKCHAIN_MAX_TRANSACTION_EXPIRATION_SEC)
            ...
            const string& alternate_path)
        {
            ...
            else
            {
				...
				if (fc::exists(alternate_path))
				{
					...
					back_path = temp_path + ".bak";
					fc::rename(temp_path, back_path);
        ...
        void Wallet::script_import_to_db(const fc::path& src_path)
        {
            try
            {
                FC_ASSERT(is_open(), "Wallet not open!");
        ...
        void Wallet::script_export_from_db(const fc::path & des_path)
        {
            try
            {
                FC_ASSERT(is_open(), "Wallet not open!");
