...
void  TransactionBuilder::set_wallet_implementation(std::unique_ptr<gdwcore::wallet::detail::WalletImpl>& wimpl)
{
    _wimpl = wimpl.get();
}
    ...
    ShareType     amount_to_receiver)
{
    try {
        ...
        if (trx.expiration == time_point_sec())
            trx.expiration = gdwcore::consensus::now() + _wimpl->self->get_transaction_expiration();
...
TransactionBuilder& TransactionBuilder::deposit_asset(const gdwcore::wallet::WalletAccountEntry& payer,
    const gdwcore::consensus::AccountEntry& recipient,
    const Asset& amount,
    const string& memo,
    ...
    const string& gdw_account)
{
    try {
        ...
        if (memo.size() > GDW_BLOCKCHAIN_MAX_EXTENDED_MEMO_SIZE)
...
TransactionBuilder& TransactionBuilder::deposit_asset_with_escrow(const gdwcore::wallet::WalletAccountEntry& payer,
    const gdwcore::consensus::AccountEntry& recipient,
    const gdwcore::consensus::AccountEntry& escrow_agent,
    ...
    const string& gdw_account)
{
    try {
        ...
        // Don't automatically truncate memos as long as users still depend on them via deposit ops rather than mail
        if (memo.size() > GDW_BLOCKCHAIN_MAX_MEMO_SIZE)
            FC_CAPTURE_AND_THROW(memo_too_long, (memo));
    ...
    )
{
    try {
        ...
        if (market_fee >= 0 || market_fee <= GDW_BLOCKCHAIN_MAX_UIA_MARKET_FEE)
            actual_market_fee = GDW_BLOCKCHAIN_MAX_UIA_MARKET_FEE * market_fee;
...
WalletTransactionEntry& TransactionBuilder::sign()
{
    try {
		...
		FC_ASSERT(i > 0, "Sign failed,maybe you don't own priv key which is required!");
