...
using namespace gdwcore::wallet::detail;
...
bool gdwcore::wallet::detail::WalletImpl::scan_withdraw_contract(const WithdrawContractOperation & op, WalletTransactionEntry & trx_rec, Asset & total_fee, PublicKeyType key, bool bNeedcreate)
{
	try {
...
bool gdwcore::wallet::detail::WalletImpl::scan_ondestroycontract_withdraw(const OnDestroyOperation & op, WalletTransactionEntry & trx_rec, Asset & total_fee, PublicKeyType key, bool bNeedcreate)
{
	try {
		const auto amount = Asset(op.amount); // Always base asset
...
bool gdwcore::wallet::detail::WalletImpl::scan_ondestroycontract_deposit(const OnDestroyOperation & op, WalletTransactionEntry & trx_rec, Asset & total_fee, bool bHaswithdraw)
{
	try {
            ...
            else /* non-TITAN with no memo, market cancel, or cover proceeds */
            {
                ...
                if (okey_rec && okey_rec->has_private_key())
                {
                    ...
                    if (new_entry)
                    {
                        auto entry = LedgerEntry();
                        //entry.from_account = okey_rec->public_key;
						//entry.memo = "deposit to" + string(okey_rec->public_key);
...
bool gdwcore::wallet::detail::WalletImpl::scan_deposit_contract(const DepositContractOperation & op, WalletTransactionEntry & trx_rec, Asset & total_fee, bool bHaswithdraw)
{
	try{
		auto entry = LedgerEntry();
...
void Wallet::cache_transaction(WalletTransactionEntry& transaction_entry, bool store)
{
    try {
		...
		if (store)
		{
			...
			transaction_entry.created_time = gdwcore::consensus::now();
    ...
    TrxType trx_search_type)const
{
    try {
        ...
        if (!asset_symbol.empty() && asset_symbol != GDW_BLOCKCHAIN_SYMBOL)
        {
            try
            {
        ...
        for (const auto& item : transactions)
        {
            ...
            if (!tx_entry.is_confirmed && (tx_entry.trx.expiration < gdwcore::consensus::now()))
            {
                continue;
            }
...
PrettyTransaction		Wallet::to_pretty_trx(const gdwcore::consensus::TransactionEntry& trx_entry, const std::string addr_for_fee) const
{
    ...
    pretty_trx.trx_type = gdwcore::consensus::TransactionType::normal_transaction;
    ...
    for (const auto & op : trx_entry.trx.operations)
    {
		...
		case withdraw_pay_op_type:
		{
            ...
            pretty_trx.trx_type = gdwcore::consensus::TransactionType::withdraw_pay_transaction;
			break;
		}
    ...
    for (const auto& op : trx.operations)
    {
        ...
        case register_account_op_type:
        {
            ...
            if (register_op.delegate_pay_rate > 100)
            {
                ...
                pretty_trx.trx_type = gdwcore::consensus::TransactionType::register_account_transaction;
            }
            else
            {
                pretty_entry.memo = "register delegate " + account_name_entry->name; // Can't tell if initially registered as a delegate
                pretty_trx.trx_type = gdwcore::consensus::TransactionType::register_delegate_transaction;
            }
		...
		case update_account_op_type:
		{
            ...
            //所以现在只能简单粗暴得用一个1000GDW来进行划分，只要是大于1000GDW都认为是升级代理
            if (total_fee <= Asset( 1000 * GDW_BLOCKCHAIN_PRECISION ))
            {
                pretty_entry.memo = "update " + account_name_entry->name;
                pretty_trx.trx_type = gdwcore::consensus::TransactionType::update_account_transaction;
            }
            else
            {
                pretty_entry.memo = "upgrade " + account_name_entry->name;
                pretty_trx.trx_type = gdwcore::consensus::TransactionType::upgrade_account_transaction;
            }
...
PrettyContractTransaction		Wallet::to_pretty_contract_trx(const gdwcore::consensus::TransactionEntry& trx_entry) const
{
    PrettyContractTransaction pretty_trx;
    SignedTransaction result_trx = trx_entry.trx;

    FC_ASSERT((trx_entry.trx.operations.size() > 0) && (trx_entry.trx.operations[0].type == gdwcore::consensus::OperationTypeEnum::transaction_op_type) , 
        "not result contract transaction");
    SignedTransaction orig_trx = trx_entry.trx.operations[0].as<gdwcore::consensus::TransactionOperation>().trx;
    ...
    FC_ASSERT((orig_trx.operations.size() > 0) && 
        ((orig_trx.operations[0].type >= fc::enum_type<uint8_t, gdwcore::consensus::OperationTypeEnum>(contract_register_op_type))
            && (orig_trx.operations[0].type <= fc::enum_type<uint8_t, gdwcore::consensus::OperationTypeEnum>(transfer_contract_op_type))), "invalid original contract transaction");


    gdwcore::consensus::OperationTypeEnum contract_op_type = orig_trx.operations[0].type;
    ...
    if (contract_op_type == gdwcore::consensus::OperationTypeEnum::contract_register_op_type)
    {
        pretty_trx.trx_type = gdwcore::consensus::TransactionType::contract_register_transaction;

        PrettyContractLedgerEntry to_contract_ledger_entry;
        vector<PrettyContractLedgerEntry> from_contract_ledger_entries;

        gdwcore::consensus::RegisterContractOperation contract_register_op = orig_trx.operations[0].as<gdwcore::consensus::RegisterContractOperation>();
        Address contract_owner = gdwcore::consensus::Address(contract_register_op.owner);
        ContractIdType contract_id = contract_register_op.get_contract_id();
        ...
        to_contract_ledger_entry.amount = Asset(GDW_DEFAULT_CONTRACT_MARGIN, 0);
        ...
        {
            // 注册的合约成功上链
            if (op.type == fc::enum_type<uint8_t, gdwcore::consensus::OperationTypeEnum>(contract_info_op_type))
                register_success = true;
            
            if (op.type == fc::enum_type<uint8_t, gdwcore::consensus::OperationTypeEnum>(balances_withdraw_op_type))
            {
    ...
    else if (contract_op_type == gdwcore::consensus::OperationTypeEnum::contract_upgrade_op_type)
    {
        pretty_trx.trx_type = gdwcore::consensus::TransactionType::contract_upgrade_transaction;

        PrettyContractLedgerEntry to_contract_ledger_entry;
        vector<PrettyContractLedgerEntry> from_contract_ledger_entries;

        gdwcore::consensus::UpgradeContractOperation contract_upgrade_op = orig_trx.operations[0].as<gdwcore::consensus::UpgradeContractOperation>();
        ContractIdType contract_id = contract_upgrade_op.id;
        ...
        {
            // 合约成功升级
            if (op.type == fc::enum_type<uint8_t, gdwcore::consensus::OperationTypeEnum>(on_upgrade_op_type))
            ...
            // 合约出账(含保证金) 
            if (op.type == fc::enum_type<uint8_t, gdwcore::consensus::OperationTypeEnum>(withdraw_contract_op_type))
            {
            ...
            // 用户入账
            if (op.type == fc::enum_type<uint8_t, gdwcore::consensus::OperationTypeEnum>(deposit_op_type))
            {
    ...
    else if (contract_op_type == gdwcore::consensus::OperationTypeEnum::contract_destroy_op_type)
    {
        pretty_trx.trx_type = gdwcore::consensus::TransactionType::contract_destroy_transaction;

        PrettyContractLedgerEntry to_contract_ledger_entry;
        vector<PrettyContractLedgerEntry> from_contract_ledger_entries;

        gdwcore::consensus::DestroyContractOperation contract_destroy_op = orig_trx.operations[0].as<gdwcore::consensus::DestroyContractOperation>();
        ContractIdType contract_id = contract_destroy_op.id;
        ...
        {
            // 合约成功销毁
            if (op.type == fc::enum_type<uint8_t, gdwcore::consensus::OperationTypeEnum>(on_destroy_op_type))
            {
    ...
    else if (contract_op_type == gdwcore::consensus::OperationTypeEnum::contract_call_op_type)
    {
        pretty_trx.trx_type = gdwcore::consensus::TransactionType::contract_call_transaction;

        PrettyContractLedgerEntry to_contract_ledger_entry;
        vector<PrettyContractLedgerEntry> from_contract_ledger_entries;

        gdwcore::consensus::CallContractOperation contract_call_op = orig_trx.operations[0].as<gdwcore::consensus::CallContractOperation>();
        Address contract_caller = gdwcore::consensus::Address(contract_call_op.caller);
        ...
        {
            // 合约成功调用
            if (op.type == fc::enum_type<uint8_t, gdwcore::consensus::OperationTypeEnum>(on_call_success_op_type))
                call_success = true;
    ...
    else if (contract_op_type == gdwcore::consensus::OperationTypeEnum::transfer_contract_op_type)
    {
        pretty_trx.trx_type = gdwcore::consensus::TransactionType::transfer_contract_transaction;

        PrettyContractLedgerEntry to_contract_ledger_entry;
        vector<PrettyContractLedgerEntry> from_contract_ledger_entries;

        gdwcore::consensus::TransferContractOperation transfer_contract_op = orig_trx.operations[0].as<gdwcore::consensus::TransferContractOperation>();
        Address contract_depositer = gdwcore::consensus::Address(transfer_contract_op.from);
        ...
        {
            // 向合约成功充值
            if (op.type == fc::enum_type<uint8_t, gdwcore::consensus::OperationTypeEnum>(deposit_contract_op_type))
