    ...
    namespace wallet {
        ...
        namespace detail {
            ...
            public:
                ...
                gdwcore::db::LevelMap<int32_t, GenericWalletEntry> _entrys;
        ...
        PublicKeyType WalletDb::generate_new_account(const fc::sha512& password, const string& account_name, const variant& private_data)
        {
            try {
                ...
                account.set_active_key(gdwcore::consensus::now(), active_public_key);
                account.last_update = gdwcore::consensus::now();
        ...
        void WalletDb::store_account(const gdwcore::consensus::AccountEntry& blockchain_account_entry, fc::sha512 password)
        {
            try {
                FC_ASSERT(is_open(), "Wallet not open!");
                ...
                gdwcore::consensus::AccountEntry& temp = *account_entry;
		...
		void WalletDb::store_contract(const gdwcore::consensus::ContractEntry& blockchain_contract_entry)
		{
			try
			{
				FC_ASSERT(is_open(), "Wallet not open!");
