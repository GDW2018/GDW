...
namespace gdwcore {
    namespace consensus {
        ...
        void TransactionEvaluationState::validate_required_fee(Asset contract_asset)
        {
            try {
                Asset xts_fees;
        ...
        void TransactionEvaluationState::post_evaluate()
        {
            try {
                ...
                /* if( _current_state->get_head_block_num() >= GDW_V0_6_4_FORK_BLOCK_NUM )
                {
                for( const auto& op : trx.operations )
                {
                if( operation_type_enum( op.type ) == cover_op_type )
                {
                FC_ASSERT( balance[ 0 ] <= GDW_BLOCKCHAIN_PRECISION );
                }
                }
                }*/
            } FC_CAPTURE_AND_RETHROW()
        ...
        bool TransactionEvaluationState::transaction_entry_analy(const SignedTransaction& trx_arg, std::vector<BalanceEntry>& all_balances, std::vector<AccountEntry>& all_account)
        {
            for (const auto& op : trx.operations)
            {
                    ...
                    || op.type.value == contract_upgrade_op_type || op.type.value == contract_destroy_op_type)
                {
                    ...
                    {
                        auto contract_register_op = op.as<RegisterContractOperation>();
                        balances = contract_register_op.balances;
                    }
                    else if (op.type.value == contract_call_op_type)
                    {
                        auto contract_call_op = op.as<CallContractOperation>();
                        balances = contract_call_op.balances;
                    }
                    ...
                    {
                        auto contract_upgrade_op = op.as<UpgradeContractOperation>();
                        balances = contract_upgrade_op.balances;
                    }
                    else if (op.type.value == contract_destroy_op_type)
                    {
                        auto contract_destroy_op = op.as<DestroyContractOperation>();
                        balances = contract_destroy_op.balances;
                    }
        ...
        void TransactionEvaluationState::evaluate(const SignedTransaction& trx_arg)
        {
            ...
            try {
                ...
                try {
                    ...
                    if ((_current_state->now() + GDW_BLOCKCHAIN_BLOCK_INTERVAL_SEC + GDW_BLOCKCHAIN_MAX_TRANSACTION_EXPIRATION_SEC) < trx_arg.expiration)
                        FC_CAPTURE_AND_THROW(invalid_transaction_expiration, (trx_arg)(_current_state->now()));

                    if (origin_trx_basic_verify(trx) && (trx.data_size() > GDW_BLOCKCHAIN_MAX_TRX_SIZE))
                    {
                    ...
                    {
                        if (
                            (op.type.value >= contract_register_op_type && op.type.value <= transfer_contract_op_type)
					...
					if (num_of_signature > GDW_BLOCKCHAIN_MAX_SIGNAGTURE_NUM)
					{
						FC_CAPTURE_AND_THROW(too_much_signature, (GDW_BLOCKCHAIN_MAX_SIGNAGTURE_NUM)(num_of_signature));
					}
					...
					int signum_to_charge = num_of_signature - GDW_BLOCKCHAIN_FREESIGNATURE_LIMIT;
					if (signum_to_charge>=0)
					{
						required_fees.amount += signum_to_charge*GDW_BLOCKCHAIN_EXTRA_SIGNATURE_FEE;
					}
					/*ShareType fee_to_minus;
					if (!contract_address.empty()){
						ShareType fee_balance = ShareType(balance[0] * 7 / 10);
						fee_to_minus = fee_balance;
						auto contract = _current_state->get_contract_entry(Address(contract_address, AddressType::contract_address));
						FC_ASSERT(contract.valid(),"Contract found error");
						oContractCreatorEntry contact_fee_collect = _current_state->lookup<ContractCreatorEntry>(Address(contract->owner).AddressToString());
						if (contact_fee_collect.valid()){
							fee_balance += contact_fee_collect->fee_collector.amount;
						}
        ...
        SignedTransaction TransactionEvaluationState::simulator_evaluate(const SignedTransaction &trx_arg, bool& ignore_check_required_fee)
        {
            try
            {
                trx = trx_arg;
                ...
                try
                {
                    ...
                    if ((_current_state->now() + GDW_BLOCKCHAIN_MAX_TRANSACTION_EXPIRATION_SEC) < trx_arg.expiration)
                        FC_CAPTURE_AND_THROW(invalid_transaction_expiration, (trx_arg)(_current_state->now()));
					...
					if (trx.operations.size() == 1 &&
						(trx.operations.front().type.value >= contract_register_op_type	&& trx.operations.front().type.value <= transfer_contract_op_type))
					{
						ignore_check_required_fee = true;
					}
        ...
        bool TransactionEvaluationState::is_contract_op(const gdwcore::consensus::OperationTypeEnum& op_type)const
        {
            if (op_type >= gdwcore::consensus::OperationTypeEnum::contract_register_op_type
                && op_type <= gdwcore::consensus::OperationTypeEnum::transfer_contract_op_type)
                return true;
            return false;
        }
        ...
        bool TransactionEvaluationState::origin_trx_basic_verify(const SignedTransaction& trx)const
        {
            ...
            for (const auto& op : trx.operations)
            {
                if (op.type == gdwcore::consensus::OperationTypeEnum::transaction_op_type)
                    return false;
