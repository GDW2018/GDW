...
namespace gdwcore {
    namespace consensus {
        ...
        {
			//字段result_trx_type与result_trx_id不参与计算digest
            ...
            if (operations.size() >= 1)
            {
                ...
                if (first_op.type == transaction_op_type)
                {
                    ...
                    if (trx.operations.size() != 1)
                        return false;
                    if (trx.operations[0].type != contract_register_op_type
                        && trx.operations[0].type != contract_call_op_type
                        && trx.operations[0].type != transfer_contract_op_type
						&& trx.operations[0].type != contract_destroy_op_type
						&& trx.operations[0].type != contract_upgrade_op_type)
                        return false;
                }
                else
                {
            ...
            uint64_t precision)
        {
            ...
            FC_ASSERT(max_share_supply <= GDW_BLOCKCHAIN_MAX_SHARES, "Max_share_supply must not be bigger than GDW_BLOCKCHAIN_MAX_SHARES!");
        ...
        void Transaction::contract_call(const ContractIdType& contract, const string& method, const string& arguments, const PublicKeyType& caller_public_key, const Asset& costlimit,const Asset& transaction_fee, const map<BalanceIdType, ShareType>& balances)
        {
        ...
        gdwcore::consensus::ContractIdType Transaction::contract_register(const uvm::blockchain::Code& code, const PublicKeyType& owner, const Asset& initlimit,const Asset& transaction_fee, const map<BalanceIdType, ShareType>& balances)
        {
        ...
        void Transaction::contract_upgrade(const ContractIdType& id,
		...
		void Transaction::contract_destroy(const ContractIdType& id, Asset &exec_limit, const Asset& fee, const std::map<BalanceIdType, ShareType>& balances)
        {
