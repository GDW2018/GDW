...
namespace gdwcore {
    namespace consensus {
        ...
        {
            const ChainInterfacePtr prev_state = _prev_state.lock();
            if (!prev_state) return get_slot_start_time(gdwcore::consensus::now());
        ...
        void PendingChainState::apply_changes()const
        {
            ...
            //contract related
			apply_entrys(prev_state, _bytecode_hash_permitted, _bytecode_hash_remove);
        ...
        TransactionEvaluationStatePtr   PendingChainState::simulator_evaluate_transaction(const SignedTransaction& trx, const ShareType required_fees)
        {
            try
            {
				...
				SignedTransaction result_trx = trx_eval_state->simulator_evaluate(trx, no_check_required_fee);
                ...
                if (result_trx.operations.size() != 0)
                {
					...
					trx_eval_state->simulator_evaluate(result_trx, no_check_required_fee);
        ...
        void PendingChainState::get_undo_state(const ChainInterfacePtr& undo_state_arg)const
        {
            ...
            //contract related
			populate_undo_state(undo_state, prev_state, _bytecode_hash_permitted, _bytecode_hash_remove);
        ...
        gdwcore::consensus::BlockIdType PendingChainState::get_block_id(uint32_t block_num) const
        {
            const ChainInterfacePtr prev_state = _prev_state.lock();
        ...
        gdwcore::consensus::SignedBlockHeader PendingChainState::get_block_header(const BlockIdType& id) const
        {
            const ChainInterfacePtr prev_state = _prev_state.lock();
		...
		gdwcore::consensus::oContractTemplateEntry PendingChainState::contracttemplate_lookup_by_hash(const std::string& hash)const
		{
			auto it = _bytecode_hash_permitted.find(hash);
			if (it != _bytecode_hash_permitted.end())
				return it->second;
			if (_bytecode_hash_remove.count(hash) > 0)
				return oContractTemplateEntry();
			const ChainInterfacePtr prev_state = _prev_state.lock();
			if (!prev_state)
				return oContractTemplateEntry();
			return prev_state->lookup<ContractTemplateEntry>(hash);
		}

		void PendingChainState::contracttemplate_insert_into_hash_map(const std::string& hash, const ContractTemplateEntry& entry)
		{
			_bytecode_hash_remove.erase(hash);
			_bytecode_hash_permitted[hash] = entry;
		}

		void PendingChainState::contracttemplate_erase_from_hash_map(const std::string& hash)
		{
