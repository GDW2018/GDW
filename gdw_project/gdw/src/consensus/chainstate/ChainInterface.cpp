...
namespace gdwcore {
    namespace consensus {
        ...
        {
            try {
                if (name.size() < GDW_BLOCKCHAIN_MIN_NAME_SIZE) return false;
                if (name.size() > GDW_BLOCKCHAIN_MAX_NAME_SIZE) return false;
        ...
        {
            try {
                // if( get_head_block_num() < GDW_V0_5_0_FORK_BLOCK_NUM )
                // return is_valid_symbol_name_v1( symbol );

                FC_ASSERT(symbol != "GDW", "Symbol must not be GDW");

                if (symbol.size() < GDW_BLOCKCHAIN_MIN_SYMBOL_SIZE)
                ...
                if (dots == 0)
                {
                    if (position > GDW_BLOCKCHAIN_MAX_SGDW_SYMBOL_SIZE)
                        FC_ASSERT(false, "Symbol name too large");
                }
                else //dots == 1 means hierarchial asset name
                {
                    if (position - dot_position - 1 > GDW_BLOCKCHAIN_MAX_SGDW_SYMBOL_SIZE)
                        FC_ASSERT(false, "Sub-symbol name too large");
                    if (symbol.size() > GDW_BLOCKCHAIN_MAX_SYMBOL_SIZE)
                        FC_ASSERT(false, "Symbol name too large");
                }

                if (symbol.find("GDW") == 0)
                    FC_ASSERT(false, "Symbol names cannot be prefixed with GDW");
        ...
        {
			return ShareType(0);
            ShareType pay_per_block = GDW_MAX_DELEGATE_PAY_PER_BLOCK;
			const auto share_entry = get_asset_entry(GDW_BLOCKCHAIN_SYMBOL);
        ...
        {
            // if( get_head_block_num() < GDW_V0_4_24_FORK_BLOCK_NUM )
            // return get_delegate_registration_fee_v1( pay_rate );

            if (pay_rate == 0) return 0;
			return ShareType((int64_t)1000000 * GDW_BLOCKCHAIN_PRECISION);
            ...
            const ShareType max_pay_per_two_weeks = max_total_pay_per_two_weeks / GDW_BLOCKCHAIN_NUM_DELEGATES;
        ...
        {
            if (imessage.size() < GDW_BLOCKCHAIN_MAX_FREE_MESSAGE_SIZE)
            {
                return 0;
            }
            ImessageIdType fee_coe = GDW_BLOCKCHAIN_MIN_MESSAGE_FEE_COE;
            ShareType fee = 0;
            fee = fee_coe * (imessage.size() - GDW_BLOCKCHAIN_MAX_FREE_MESSAGE_SIZE);
            return fee;
        }
        ShareType ChainInterface::get_asset_registration_fee(uint8_t symbol_length)const
        {
            ...
            static const ShareType long_symbol_price = 500 * GDW_BLOCKCHAIN_PRECISION; //
        ...
        double ChainInterface::to_pretty_price_double(const Price& price_to_pretty_print)const
        {
            ...
            return fc::variant(string(price_to_pretty_print.ratio * obase_asset->precision / oquote_asset->precision)).as_double() / (GDW_BLOCKCHAIN_MAX_SHARES * 1000);
        }
        ...
        uint64_t ChainInterface::get_required_confirmations()const
        {
            try {
                ...
                return GDW_BLOCKCHAIN_NUM_DELEGATES * 3;
            } FC_CAPTURE_AND_RETHROW()
        }
        ...
        gdwcore::consensus::Asset ChainInterface::get_amount(ShareType limit, AssetIdType asset_id)
        {
        ...
        {
            Asset fee(GDW_DEFAULT_TRANSACTION_FEE, 0);
            return fee;
        }

        Asset ChainInterface::get_contract_register_fee(const uvm::blockchain::Code& code) const
        {
        ...
        {
            Asset fee(GDW_DEFAULT_CONTRACT_MARGIN, 0);
            return fee;
        }
        ...
        {
            return GDW_DEFAULT_TRANSACTION_EXPIRATION_SEC;
        }
        */
        ...
        {
            //BalanceIdType balance_id = contract_get_balanceid(from_contract_address);
		...
		oContractTemplateEntry			  ChainInterface::get_contract_template_entry(const std::string& hash) const
		{
			try {
				return lookup<ContractTemplateEntry>(hash);
			} FC_CAPTURE_AND_RETHROW((hash))
		}

		void                   ChainInterface::remove_contract_template_entry(const std::string& hash)
		{
			try {
				return remove<ContractTemplateEntry>(hash);
			} FC_CAPTURE_AND_RETHROW((hash))
		}

		void                   ChainInterface::store_contract_template_entry(const ContractTemplateEntry& entry)
		{
			try {
				store(entry.code.code_hash, entry);
			} FC_CAPTURE_AND_RETHROW((entry))
		}
            ...
            const Address& to_account_address, SignedTransaction& trx)
        {
            try {
				...
				{
					FC_CAPTURE_AND_THROW(invalid_asset_symbol,("Only GDW Allowed"));
				}
