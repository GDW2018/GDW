...
#include <consensus/chainstate/ChainDatabaseImpl.hpp>
...
namespace gdwcore {
    namespace consensus {
        ...
        namespace detail
        {
            void ChainDatabaseImpl::revalidate_pending()
            {
                ...
                while (itr.valid())
                {
                    ...
                    try
                    {
                        ...
                        count++;
                        if (count > GDW_BLOCKCHAIN_REVALIDATE_MAX_TRX_COUNT)
                            break;
            ...
            bool ChainDatabaseImpl::replay_required(const fc::path& data_dir)
            {
                try {
                    ...
                    return !entry.valid() || entry->value.as_uint64() != GDW_BLOCKCHAIN_DATABASE_VERSION;
                } FC_CAPTURE_AND_RETHROW((data_dir))
            }
            ...
            void ChainDatabaseImpl::open_database(const fc::path& data_dir)
            {
                try {
                    ...
                    //contract db
					_bytecode_hash_permitted.open(data_dir / "index/bytecode_hash_permitted");
            ...
            void ChainDatabaseImpl::clear_invalidation_of_future_blocks()
            {
                for (auto block_id_itr = _revalidatable_future_blocks_db.begin(); block_id_itr.valid();)
                {
                    ...
                    fc::time_point_sec now = gdwcore::consensus::now();
            ...
            DigestType ChainDatabaseImpl::initialize_genesis(const optional<path>& genesis_file, const bool statistics_enabled)
            {
                try {
                    ...
                    // Check genesis state
                    FC_ASSERT(config.delegates.size() >= GDW_BLOCKCHAIN_NUM_DELEGATES,
                        "genesis.json does not contain enough initial delegates!",
                        ("required", GDW_BLOCKCHAIN_NUM_DELEGATES)("provided", config.delegates.size()));
                    ...
                    const auto convert_raw_address = [](const string& raw_address) -> Address
                    {
                        ...
                        catch (const fc::exception&)
                        {
                            ...
                            //  {
                            //  return address( GDW_ADDRESS_PREFIX + raw_address.substr( prefix.size() ) );
                            // }
                            //}
                        }
                    ...
                    base_asset.symbol = GDW_BLOCKCHAIN_SYMBOL;
                    base_asset.name = GDW_BLOCKCHAIN_NAME;
                    base_asset.description = GDW_BLOCKCHAIN_DESCRIPTION;
                    base_asset.public_data = variant("");
                    base_asset.issuer_account_id = 0;
                    base_asset.precision = GDW_BLOCKCHAIN_PRECISION;
                    base_asset.registration_date = timestamp;
                    base_asset.last_update = timestamp;
                    base_asset.current_share_supply = total_base_supply;
                    base_asset.maximum_share_supply = GDW_BLOCKCHAIN_MAX_SHARES;
                    ...
                    for (const GenesisAsset& asset : config.market_assets)
                    {
                        ...
                        rec.maximum_share_supply = GDW_BLOCKCHAIN_MAX_SHARES;
                ...
                const FullBlock& block_data)
            {
                try {
                    ...
                    if (self->get_statistics_enabled())
                    {
                        ...
                        entry.latency = gdwcore::consensus::now() - block_data.timestamp;
                ...
                const PendingChainStatePtr& pending_state)
            {
                try {
                    ...
                    for (const auto& trx : block_data.user_transactions)
                    {
                        ...
                        FC_ASSERT(trx.data_size() <= GDW_BLOCKCHAIN_MAX_TRX_SIZE, "trx size is out of the max trx size range");
            ...
            {
                try {
                    // if( pending_state->get_head_block_num() < GDW_V0_6_0_FORK_BLOCK_NUM )
                    ...
                    static const uint32_t blocks_per_two_weeks = 14 * GDW_BLOCKCHAIN_BLOCKS_PER_DAY;
                ...
                const PendingChainStatePtr& pending_state)
            {
                try {
                    ...
                    if (block_num > GDW_BLOCKCHAIN_MAX_UNDO_HISTORY)
                    {
                        const uint32_t old_block_num = block_num - GDW_BLOCKCHAIN_MAX_UNDO_HISTORY;
            ...
            void ChainDatabaseImpl::verify_header(const DigestBlock& block_digest, const PublicKeyType& block_signee)const
            {
                try {
                    ...
                    if (block_digest.timestamp.sec_since_epoch() % GDW_BLOCKCHAIN_BLOCK_INTERVAL_SEC != 0)
                    ...
                    if (block_digest.timestamp > (now + GDW_BLOCKCHAIN_BLOCK_INTERVAL_SEC * 2))
            ...
            /**
             *  A block should be produced every GDW_BLOCKCHAIN_BLOCK_INTERVAL_SEC. If we do not have a
                ...
                const PendingChainStatePtr& pending_state)const
            {
                try {
                    ...
                    if (head_block.block_num > 0) block_timestamp = head_block.timestamp + GDW_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
                    ...
                    for (; block_timestamp < block_header.timestamp;
                        block_timestamp += GDW_BLOCKCHAIN_BLOCK_INTERVAL_SEC,
                    ...
                    if (required_confirmations > GDW_BLOCKCHAIN_NUM_DELEGATES * 3)
                        required_confirmations = 3 * GDW_BLOCKCHAIN_NUM_DELEGATES;
            ...
            {
                try {//todo remove????
                    // if( pending_state->get_head_block_num() < GDW_V0_7_0_FORK_BLOCK_NUM )
                    // return update_active_delegate_list_v1( block_num, pending_state );

                    if (block_num % GDW_BLOCKCHAIN_NUM_DELEGATES != 0)
                        return;
            ...
            void ChainDatabaseImpl::extend_chain(const FullBlock& block_data)
            {
                try {
                    ...
                    try
                    {
                        ...
                        if (gdwcore::client::g_client->get_wallet() != nullptr&&gdwcore::client::g_client->get_wallet()->is_open())
                        {
                            if (!gdwcore::client::g_client->get_wallet()->get_my_delegates(gdwcore::wallet::enabled_delegate_status).empty())
                            {
                                auto fee = gdwcore::client::g_client->get_delegate_config().transaction_min_fee;
                                _relay_fee = fee > GDW_BLOCKCHAIN_DEFAULT_RELAY_FEE ? fee : GDW_BLOCKCHAIN_DEFAULT_RELAY_FEE;
                            }
                            else
                            {
                                auto fee = gdwcore::client::g_client->get_wallet()->get_transaction_fee();
                                _relay_fee = fee.amount > GDW_BLOCKCHAIN_DEFAULT_RELAY_FEE ? fee.amount : GDW_BLOCKCHAIN_DEFAULT_RELAY_FEE;
                                //_relay_fee = GDW_BLOCKCHAIN_DEFAULT_RELAY_FEE;
                            }
                        }
                    }
                    catch (const fc::exception& e)
                    ...
                    if ((now() - block_data.timestamp).to_seconds() < GDW_BLOCKCHAIN_BLOCK_INTERVAL_SEC)
            ...
            void ChainDatabaseImpl::pop_block()
            {
                try {
                    ...
                    gdwcore::consensus::PendingChainStatePtr undo_state_ptr = std::make_shared<gdwcore::consensus::PendingChainState>(undo_state);
        ...
        {
            return get_delegates_by_vote(0, GDW_BLOCKCHAIN_NUM_DELEGATES);
        }
            ...
            try {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        my->open_database(data_dir);
                        store_property_entry(PropertyIdType::database_version, variant(GDW_BLOCKCHAIN_DATABASE_VERSION));
                        ...
                        const auto toggle_leveldb = [this](const bool enabled)
                        {
                            ...
                            //contract db related
							my->_bytecode_hash_permitted.toggle_leveldb(enabled);
                        ...
                        const auto start_time = gdwcore::consensus::now();
                        ...
                        const auto insert_block = [&](const FullBlock& block)
                        {
                            if (blocks_indexed % 200 == 0)
                            {
                                ...
                                else
                                {
                                    ...
                                    progress = float(blocks_indexed * GDW_BLOCKCHAIN_BLOCK_INTERVAL_SEC) / seconds;
                                }
                        ...
                        try
                        {
                            ...
                            else
                            {
                                ...
                                if (last_known_block_num > GDW_BLOCKCHAIN_MAX_UNDO_HISTORY)
                                    my->_min_undo_block = last_known_block_num - GDW_BLOCKCHAIN_MAX_UNDO_HISTORY;
                        ...
                        catch (gdwcore::consensus::store_and_index_a_seen_block&  e)
                        {
                        ...
                        std::cout << "\rSuccessfully replayed " << blocks_indexed << " blocks in "
                            << (gdwcore::consensus::now() - start_time).to_seconds() << " seconds.                          "
        ...
        void ChainDatabase::close()
        {
            try {
                ...
                //contract db
				my->_bytecode_hash_permitted.close();
        ...
        {
            try {
                auto slot_number = gdwcore::consensus::get_slot_number(timestamp);
                auto delegate_pos = slot_number % GDW_BLOCKCHAIN_NUM_DELEGATES;
        ...
        {
            try {
                auto next_block_time = gdwcore::consensus::get_slot_start_time(gdwcore::consensus::now());
                if (next_block_time <= now()) next_block_time += GDW_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
                auto last_block_time = next_block_time + (GDW_BLOCKCHAIN_NUM_DELEGATES * GDW_BLOCKCHAIN_BLOCK_INTERVAL_SEC);

                auto active_delegates = get_active_delegates();
                for (; next_block_time < last_block_time; next_block_time += GDW_BLOCKCHAIN_BLOCK_INTERVAL_SEC)
                {
                    auto slot_number = gdwcore::consensus::get_slot_number(next_block_time);
                    auto delegate_pos = slot_number % GDW_BLOCKCHAIN_NUM_DELEGATES;
            ...
            const ShareType required_fees, bool contract_vm_exec, bool skip_signature_check, bool throw_exec_exception)
        {
            try {
                ...
                try
                {
					trx_eval_state->ignore_state = false;
                    trx_eval_state->evaluate(trx);
                }
                catch (gdwcore::consensus::ignore_check_required_fee_state& e)
                {
        ...
        BlockForkData ChainDatabase::push_block(const FullBlock& block_data)
        {
            try {
                ...
                if (head_block_num > GDW_BLOCKCHAIN_MAX_UNDO_HISTORY &&
                    block_data.block_num <= (head_block_num - GDW_BLOCKCHAIN_MAX_UNDO_HISTORY))
                {
                    elog("block ${new_block_hash} (number ${new_block_num}) is on a fork older than "
                        ...
                        ("head_block_num", head_block_num)("undo_history", GDW_BLOCKCHAIN_MAX_UNDO_HISTORY));
                ...
                */
                ...
                else
                {
                    ...
                    gdwcore::consensus::ntp_time();
                    // 		 gdwcore::consensusupdate_ntp_time();
        ...
        fc::time_point_sec ChainDatabase::now()const
        {
            ...
            {
                auto slot_number = gdwcore::consensus::get_slot_number(gdwcore::consensus::now());
                return gdwcore::consensus::get_slot_start_time(slot_number - 1);
            }
        ...
        */
        double ChainDatabase::get_average_delegate_participation()const
        {
            try {
                ...
                const auto now = gdwcore::consensus::now();
                if (head_num < 1)
                {
                    return 0;
                }
                else if (head_num <= GDW_BLOCKCHAIN_NUM_DELEGATES)
                {
                    // what percent of the maximum total blocks that could have been produced
                    // have been produced.
                    const auto expected_blocks = (now - get_block_header(1).timestamp).to_seconds() / GDW_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
                ...
                else
                {
                    ...
                    const auto starting_time = get_block_header(head_num - GDW_BLOCKCHAIN_NUM_DELEGATES).timestamp;
                    const auto expected_production = (now - starting_time).to_seconds() / GDW_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
                    return 100 * double(GDW_BLOCKCHAIN_NUM_DELEGATES) / expected_production;
                }
            } FC_RETHROW_EXCEPTIONS(warn, "")
        }
        ...
        TransactionEvaluationStatePtr ChainDatabase::store_pending_transaction(const SignedTransaction& trx, bool override_limits, bool contract_vm_exec, bool cache)
        {
            try {
                ...
                if (!override_limits)
                {
                    if (my->_pending_fee_index.size() > GDW_BLOCKCHAIN_MAX_PENDING_QUEUE_SIZE)
                    {
                        auto overage = my->_pending_fee_index.size() - GDW_BLOCKCHAIN_MAX_PENDING_QUEUE_SIZE;
        ...
        FullBlock ChainDatabase::generate_block(const time_point_sec block_timestamp, const DelegateConfig& config)
        {
            try {
                ...
                //if( pending_state->get_head_block_num() >= GDW_V0_4_9_FORK_BLOCK_NUM )
                    ...
                    for (const TransactionEvaluationStatePtr& item : pending_trx)
                    {
                        ...
                        try
                        {
                            ...
                            auto pending_trx_state = origin_trx_state;
                            {
                                ...
                                trx_eval_state->_skip_signature_check = true;
                                trx_eval_state->skipexec = false;
								trx_eval_state->is_delegate = true;
                                ...
                                if (trx_eval_state->p_result_trx.operations.size() > 0)
                                {
									...
									result_eval_state->is_delegate = true;
                                    ...
                                    if (iMessageLength > GDW_BLOCKCHAIN_MAX_FREE_MESSAGE_SIZE)
                                    {
                                        imessage_fee = config.transaction_imessage_min_fee_coe * (iMessageLength - GDW_BLOCKCHAIN_MAX_FREE_MESSAGE_SIZE);
                                    }
                ...
                // if( new_block.block_num < GDW_V0_7_0_FORK_BLOCK_NUM )
        ...
        string ChainDatabase::export_fork_graph(uint32_t start_block, uint32_t end_block, const fc::path& filename)const
        {
            ...
            for (auto block_itr = my->_block_id_to_full_block.begin(); block_itr.valid(); ++block_itr)
            {
                ...
                {
                    unsigned rank = (unsigned)((block.timestamp - start_time).to_seconds() / GDW_BLOCKCHAIN_BLOCK_INTERVAL_SEC);
        ...
        bool ChainDatabase::get_is_in_simulator()const
        {
            return my->_is_in_simulator;
        }

        void ChainDatabase::set_is_in_simulator(bool simulator)
        {
            my->_is_in_simulator = simulator;
        }

        PendingChainStatePtr ChainDatabase::get_simulator_pending_state()
        {
            if (my->_simulator_pending_state == nullptr)
                my->_simulator_pending_state = std::make_shared<PendingChainState>(shared_from_this());

            return my->_simulator_pending_state;
        }

        void ChainDatabase::set_simulator_pending_state(PendingChainStatePtr state_ptr)
        {

            my->_simulator_pending_state = state_ptr;
        }

        void ChainDatabase::clear_simulator_pending_state()
        {
            my->_simulator_pending_state = nullptr;
        }

        bool ChainDatabase::store_balance_entries_for_simulator()
        {
            my->_simulator_pending_state->_balance_id_to_entry.insert(my->_balance_id_to_entry.unordered_begin(), my->_balance_id_to_entry.unordered_end());

            return true;
        }

        bool ChainDatabase::store_account_entries_for_simulator(const vector<gdwcore::wallet::WalletAccountEntry>& vec_account_entry)
        {
            ...
            for (auto iter = vec_account_entry.begin(); iter != vec_account_entry.end(); ++iter)
            {
                ...
                my->_simulator_pending_state->_vec_wallet_accounts.push_back(std::move(tmp));
        ...
        vector<GdwTrxidBalance> ChainDatabase::fetch_gdw_input_balance(const uint32_t & block_num)
        {
            try{
                    ...
                    iter != my->_gdw_input_balance_entry.unordered_end(); ++iter)
                {
                    ...
                    set<GdwTrxidBalance>::iterator block_iter = iter->second.lower_bound(ubTemp);
        ...
        void ChainDatabase::dump_state(const fc::path& path)const
        {
            try {
                ...
                */
				...
				my->_bytecode_hash_permitted.export_to_json(next_path);
				ulog("Dumped ${p}", ("p", next_path));
        ...
        oResultTIdEntry gdwcore::consensus::ChainDatabase::contract_lookup_resultid_by_reqestid(const TransactionIdType & id) const
        {
        ...
        void gdwcore::consensus::ChainDatabase::contract_store_resultid_by_reqestid(const TransactionIdType & req, const ResultTIdEntry & res)
        {
            my->_request_to_result_iddb.store(req, res);
        }

        void gdwcore::consensus::ChainDatabase::contract_erase_resultid_by_reqestid(const TransactionIdType & req)
        {
            my->_request_to_result_iddb.remove(req);
        }

        oRequestIdEntry gdwcore::consensus::ChainDatabase::contract_lookup_requestid_by_resultid(const TransactionIdType &id) const
        {
        ...
        void gdwcore::consensus::ChainDatabase::contract_store_requestid_by_resultid(const TransactionIdType & res, const RequestIdEntry & req)
        {
            my->_result_to_request_iddb.store(res, req);
        }

        void gdwcore::consensus::ChainDatabase::contract_erase_requestid_by_resultid(const TransactionIdType & result)
        {
            my->_result_to_request_iddb.remove(result);
        }

        oContractinTrxEntry gdwcore::consensus::ChainDatabase::contract_lookup_contractid_by_trxid(const TransactionIdType &id) const
        {
        ...
        oContractTrxEntry gdwcore::consensus::ChainDatabase::contract_lookup_trxid_by_contract_id(const ContractIdType &id) const
        {
        ...
        void gdwcore::consensus::ChainDatabase::contract_store_contractid_by_trxid(const TransactionIdType & tid, const ContractinTrxEntry & cid)
        {
            my->_trx_to_contract_iddb.store(tid, cid);
        }

        void gdwcore::consensus::ChainDatabase::contract_store_trxid_by_contractid(const ContractIdType & cid, const ContractTrxEntry & tid)
        {
            my->_contract_to_trx_iddb.store(cid, tid);
        }

        void gdwcore::consensus::ChainDatabase::contract_erase_trxid_by_contract_id(const ContractIdType &id)
        {
            my->_contract_to_trx_iddb.remove(id);
        }

        void gdwcore::consensus::ChainDatabase::contract_erase_contractid_by_trxid(const TransactionIdType & tid)
        {
		...
		oContractTemplateEntry ChainDatabase::contracttemplate_lookup_by_hash(const std::string& hash)const
		{
			...
			if (it != my->_bytecode_hash_permitted.unordered_end())
				return it->second;
			return oContractTemplateEntry();
		}

		void ChainDatabase::contracttemplate_insert_into_hash_map(const std::string& hash, const ContractTemplateEntry& entry)
		{
			my->_bytecode_hash_permitted.store(hash, entry);
		}
