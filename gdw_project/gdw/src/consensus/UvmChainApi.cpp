/**
 * lua module injector header in gdwcore
 */
    ...
    namespace lua {
        namespace api {
            ...
            */
            ...
            /**
            * when exception happened, use this api to tell gdwcore
            ...
            */
            void UvmChainApi::throw_exception(lua_State *L, int code, const char *error_format, ...)
            {
                ...
                // 如果上次的exception code为GDWCORE_API_LVM_LIMIT_OVER_ERROR, 不能被其他异常覆盖
            ...
            */
            ...
            {
                return 0; // FIXME: need fill by gdwcore api
            }

            int UvmChainApi::get_stored_contract_info(lua_State *L, const char *name, std::shared_ptr<UvmContractInfo> contract_info_ret)
            {
                gdwcore::consensus::TransactionEvaluationState* pevaluate_state = (gdwcore::consensus::TransactionEvaluationState*)uvm::lua::lib::get_lua_state_value(L, "evaluate_state").pointer_value;
                // name = lua::lib::unwrap_any_contract_name(name).c_str();
                gdwcore::consensus::oContractEntry entry = pevaluate_state->_current_state->get_contract_entry(name);
                if (!entry.valid())
                    return 0;
            ...
            {
                gdwcore::consensus::TransactionEvaluationState* pevaluate_state = (gdwcore::consensus::TransactionEvaluationState*)uvm::lua::lib::get_lua_state_value(L, "evaluate_state").pointer_value;
                gdwcore::consensus::oContractEntry entry = pevaluate_state->_current_state->get_contract_entry(gdwcore::consensus::Address(std::string(address), AddressType::contract_address));
            ...
            void UvmChainApi::get_contract_address_by_name(lua_State *L, const char *name, char *address, size_t *address_size)
            {
                ...
                */
                gdwcore::consensus::TransactionEvaluationState* eval_state_ptr =
                    (gdwcore::consensus::TransactionEvaluationState*)
                ...
                gdwcore::consensus::ChainInterface* cur_state = eval_state_ptr->_current_state;
            ...
            bool UvmChainApi::check_contract_exist_by_address(lua_State *L, const char *address)
            {
                ...
                oContractEntry entry = cur_state->get_contract_entry(gdwcore::consensus::Address(std::string(address), AddressType::contract_address));
            ...
            {
                /*
                char *filename = lutil_concat_str4("gdwcore_lua_modules", file_separator_str(), "gdwcore_contract_", name);
            ...
            /**
            * load contract lua byte stream from gdwcore api
            */
            ...
            /**
            * store contract lua module byte stream to gdwcore api
            */
            //need to delete
            /*
            ...
            */
            ...
            UvmStorageValue UvmChainApi::get_storage_value_from_uvm_by_address(lua_State *L, const char *contract_address, std::string name)
            {
                ...
                gdwcore::consensus::StorageDataType storage_data = iter->second;

                return gdwcore::consensus::StorageDataType::create_lua_storage_from_storage_data(L, storage_data);
            }
            ...
            {
                // TODO
                printf("registered storage %s[%s] to gdwcore\n", contract_name, name);
                return true;
            }
                ...
                const char *asset_type, int64_t amount)
            {
                ...
                try
                {
                    if (!Address::is_valid(contract_address, CONTRACT_ADDRESS_PREFIX))
                        return -3;
                    if (!Address::is_valid(to_addr, GDW_ADDRESS_PREFIX))
                        return -4;

                    eval_state_ptr->transfer_asset_from_contract(amount, asset_type,
                        Address(string(contract_address), AddressType::contract_address), Address(to_addr, AddressType::gdw_address));
            ...
            int64_t UvmChainApi::get_contract_balance_amount(lua_State *L, const char *contract_address, const char* asset_symbol)
            {
                ...
                try{
                    gdwcore::consensus::TransactionEvaluationState* eval_state_ptr =
                        (gdwcore::consensus::TransactionEvaluationState*)
                        (uvm::lua::lib::get_lua_state_value(L, "evaluate_state").pointer_value);
                    gdwcore::consensus::ChainInterface* cur_state;
                    ...
                    {
                        FC_CAPTURE_AND_THROW(unknown_asset_id, ("Only GDW Allowed"));
                    }
			...
			bool UvmChainApi::is_valid_address(lua_State *L, const char *address_str)
			{
				...
				return Address::is_valid(addr, GDW_ADDRESS_PREFIX) || Address::is_valid(addr, CONTRACT_ADDRESS_PREFIX);
			}
			...
			{
				return GDW_BLOCKCHAIN_SYMBOL;
			}

			uint64_t UvmChainApi::get_system_asset_precision(lua_State *L)
			{
				return GDW_BLOCKCHAIN_PRECISION;
			}
