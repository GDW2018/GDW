...
namespace gdwcore {
    namespace rpc {
        ...
        namespace detail
        {
            class RpcServerImpl : public gdwcore::rpc_stubs::CommonApiRpcServer
            {
            public:
                RpcServerConfig                                 _config;
                gdwcore::client::Client*                              _client;
                ...
                typedef std::map<std::string, gdwcore::api::MethodData> MethodMapType;
                ...
                RpcServerImpl(gdwcore::client::Client* client) :
                    _client(client),
                ...
                virtual gdwcore::api::CommonApi* get_client() const override;
                ...
                virtual void verify_simulator_is_open() const override;
                virtual void verify_connected_to_network() const override;
                virtual void store_method_metadata(const gdwcore::api::MethodData& method_metadata);

                std::string help(const std::string& command_name) const;

                std::string make_short_description(const gdwcore::api::MethodData& method_data, bool show_decription = true) const
                {
                    std::string help_string;
                    std::stringstream short_description;
                    short_description << std::setw(100) << std::left;
                    help_string = method_data.name + " ";
                    for (const gdwcore::api::ParameterData& parameter : method_data.parameters)
                    {
                        if (parameter.classification == gdwcore::api::required_positional)
                            help_string += std::string("<") + parameter.name + std::string("> ");
                        else if (parameter.classification == gdwcore::api::optional_named)
                            help_string += std::string("{") + parameter.name + std::string("} ");
                        else if (parameter.classification == gdwcore::api::required_positional_hidden)
                            continue;
                        else
                            help_string += std::string("[") + parameter.name + std::string("] ");
                    }
                    ...
                    try {
                        if (_config.rpc_user.size())
                        {
                                ...
                                _config.rpc_password != password)
                            {
                                ...
                                s.add_header("WWW-Authenticate", "Basic realm=\"gdwcore wallet\"");
                ...
                fc::http::reply::status_code handle_http_rpc(const fc::http::request& r, const fc::http::server::response& s)
                {
                    ...
                    try {
                        ...
                        if (fc::time_point(gdwcore::consensus::now()) - _last_cache_clear_time > fc::seconds(GDW_BLOCKCHAIN_BLOCK_INTERVAL_SEC))
                        {
                            _last_cache_clear_time = gdwcore::consensus::now();
                            _call_cache.clear();
                        }
                ...
                void encrypted_accept_loop(const gdwcore::consensus::PrivateKeyType& server_key)
                {
                    while (!_encrypted_accept_loop_complete.canceled())
                    {
                        gdwcore::network::StcpSocketPtr sock = std::make_shared<gdwcore::network::StcpSocket>();
                        try
                        {
                ...
                fc::variant dispatch_method_from_json_connection(fc::rpc::json_connection* con,
                    const gdwcore::api::MethodData& method_data,
                    const fc::variants& arguments)
                {
                    // ilog( "arguments: ${params}", ("params",arguments) );
                    if ((method_data.prerequisites & gdwcore::api::json_authenticated) &&
                ...
                fc::variant dispatch_authenticated_method(const gdwcore::api::MethodData& method_data,
                    const fc::variants& arguments_from_caller)
                {
                    ...
                    if (method_data.prerequisites & gdwcore::api::wallet_open)
                        verify_wallet_is_open();
                    if (method_data.prerequisites & gdwcore::api::wallet_unlocked)
                        verify_wallet_is_unlocked();
                    if (method_data.prerequisites & gdwcore::api::connected_to_network)
                        verify_connected_to_network();
                    if (method_data.prerequisites & gdwcore::api::simulator_open)
                    ...
                    {
                        if (parameter.classification == gdwcore::api::required_positional
                            || parameter.classification == gdwcore::api::required_positional_hidden)
                        {
                        ...
                        else if (parameter.classification == gdwcore::api::optional_positional)
                        {
            ...
            gdwcore::api::CommonApi* RpcServerImpl::get_client() const
            {
                return _client;
            }
            ...
            void RpcServerImpl::verify_simulator_is_open() const
            {
                if (!_client->get_chain()->get_is_in_simulator())
                    throw  rpc_simulator_open_needed_exception(FC_LOG_MESSAGE(error, "The simulator must be opened before executing this command"));
            }
            ...
            void RpcServerImpl::store_method_metadata(const gdwcore::api::MethodData& method_metadata)
            {
            ...
            std::string RpcServerImpl::help(const std::string& command_name) const
            {
                ...
                { //display detailed description of requested command
                    ...
                    if (alias_iter != _alias_map.end())
                    {
                        ...
                        const gdwcore::api::MethodData& method_data = method_iter->second;
                        help_string += "Usage:\n";
        ...
        RpcServer::RpcServer(gdwcore::client::Client* client) :
        ...
        const gdwcore::api::MethodData& RpcServer::get_method_data(const std::string& method_name)
        {
        ...
        std::vector<gdwcore::api::MethodData> RpcServer::get_all_method_data() const
        {
            std::vector<gdwcore::api::MethodData> result;
            for (const detail::RpcServerImpl::MethodMapType::value_type& value : my->_method_map)
                result.emplace_back(value.second);
            return result;
        }

        void RpcServer::register_method(gdwcore::api::MethodData data)
        {
...
#define RPC_EXCEPTION_IMPL(TYPE, ERROR_CODE, DESCRIPTION) \
  ...
  int32_t TYPE::get_rpc_error_code() const { return ERROR_CODE; }
        ...
        RPC_EXCEPTION_IMPL(rpc_misc_error_exception, -1, "std::exception is thrown during command handling")
            ...
            //simulator related
            RPC_EXCEPTION_IMPL(rpc_simulator_open_needed_exception, -101, "The simulator must be opened before executing this command")
