...
namespace gdwcore {
    namespace console {
            ...
            ClientRawPtr client, std::ostream& out)const
        {
            try
            {
                ...
                else
                {
                    ...
                    const gdwcore::api::MethodData& method_data = client->get_rpc_server()->get_method_data(method_name);
                    result_type = method_data.return_type;

                    if (result_type == "asset")
                    {
                        out << string(result.as<gdwcore::consensus::Asset>()) << "\n";
                    }
                    else if (result_type == "address")
                    {
                        out << (string)result.as<gdwcore::consensus::Address>() << "\n";
                    }
        ...
        void PrintResult::initialize()
        {
			...
			command_to_function["simulator_account_balance"] = [](std::ostream& out, const fc::variants& arguments, const fc::variant& result, ClientRawPtr client)
			{
				const auto& balances = result.as<AccountBalanceSummaryType>();
				out << pretty_balances(balances, client);
			};
            ...
            command_to_function["contract_get_info"] = &f_contract_get_info;
            command_to_function["simulator_contract_get_info"] = &f_contract_get_info;
...
#define blank4 "    "
            ...
            command_to_function["scripts_list"] = [](std::ostream& out, const fc::variants& arguments, const fc::variant& result, ClientRawPtr client)
            {
            ...
            command_to_function["script_get_info"] = [](std::ostream& out, const fc::variants& arguments, const fc::variant& result, ClientRawPtr client)
            {
                auto res = result.as<gdwcore::wallet::ScriptEntryPrintable>();
                string res_str = "";
                string res_desc = "";
            ...
            command_to_function["script_list_event_handler"] = [](std::ostream& out, const fc::variants& arguments, const fc::variant& result, ClientRawPtr client)
            {
            ...
            command_to_function["contract_compile"] = print_fc_path;
            command_to_function["script_compile"] = print_fc_path;
            command_to_function["simulator_contract_compile"] = print_fc_path;
        }
        ...
        void PrintResult::f_wallet_transfer_2(std::ostream& out, const fc::variants& arguments, const fc::variant& result, ClientRawPtr client)
        {
            ...
            for (int i = 0; i < entry.trx.operations.size(); ++i)
                if (entry.trx.operations[i].type == gdwcore::consensus::OperationTypeEnum::imessage_memo_op_type)
                {
        ...
        void PrintResult::f_blockchain_get_transaction(std::ostream& out, const fc::variants& arguments, const fc::variant& result, ClientRawPtr client)
        {
            ...
            for (int i = 0; i < trx_pair.second.trx.operations.size(); ++i)
                if (trx_pair.second.trx.operations[i].type == gdwcore::consensus::OperationTypeEnum::imessage_memo_op_type)
                {
        ...
        void PrintResult::f_blockchain_get_block_transactions(std::ostream& out, const fc::variants& arguments, const fc::variant& result, ClientRawPtr client)
        {
            ...
            for (auto iter = trx_map.begin(); iter != trx_map.end(); ++iter)
                for (int i = 0; i < iter->second.trx.operations.size(); ++i)
                    if (iter->second.trx.operations[i].type == gdwcore::consensus::OperationTypeEnum::imessage_memo_op_type)
                    {
        ...
        void PrintResult::f_contract_get_info(std::ostream& out, const fc::variants& arguments, const fc::variant& result, ClientRawPtr client)
        {
        ...
        {
            auto transactions = result.as<vector<std::pair<gdwcore::consensus::TransactionIdType, gdwcore::consensus::SignedTransaction>>>();
        ...
        {
            auto peers = result.as<std::vector<gdwcore::network::PotentialPeerEntry>>();
