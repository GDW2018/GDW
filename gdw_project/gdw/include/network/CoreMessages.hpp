...
namespace gdwcore {
    namespace network {
        ...
        const uint32_t core_protocol_version = GDW_NET_PROTOCOL_VERSION;
...
FC_REFLECT_ENUM(gdwcore::network::CoreMessageTypeEnum,
    (trx_message_type)
    (block_message_type)
    ...
    FC_REFLECT(gdwcore::network::ItemId, (item_type)
    (item_hash))
    FC_REFLECT(gdwcore::network::ItemIdsInventoryMessage, (item_type)
    (item_hashes_available))
    FC_REFLECT(gdwcore::network::BlockchainItemIdsInventoryMessage, (total_remaining_item_count)
    (item_type)
    (item_hashes_available))
    FC_REFLECT(gdwcore::network::FetchBlockchainItemIdsMessage, (item_type)
    (blockchain_synopsis))
    FC_REFLECT(gdwcore::network::FetchItemsMessage, (item_type)
    (items_to_fetch))
    FC_REFLECT(gdwcore::network::ItemNotAvailableMessage, (requested_item))
    FC_REFLECT(gdwcore::network::HelloMessage, (user_agent)
    ...
    FC_REFLECT_EMPTY(gdwcore::network::ConnectionAcceptedMessage)
    FC_REFLECT_ENUM(gdwcore::network::RejectionReasonCode, (unspecified)
    (different_chain)
    (already_connected)
    (connected_to_self)
    (not_accepting_connections)
    ...
    (invalid_hello_message)
    (client_too_old))
    FC_REFLECT(gdwcore::network::ConnectionRejectedMessage, (user_agent)
    (core_protocol_version)
    (remote_endpoint)
    (reason_code)
    (reason_string))
    FC_REFLECT_EMPTY(gdwcore::network::AddressRequestMessage)
    FC_REFLECT(gdwcore::network::AddressInfo, (remote_endpoint)
    (last_seen_time)
    (latency)
    (node_id)
    (direction)
    (firewalled))
    FC_REFLECT(gdwcore::network::AddressMessage, (addresses))
    FC_REFLECT(gdwcore::network::ClosingConnectionMessage, (reason_for_closing)
    (closing_due_to_error)
    (error))
    FC_REFLECT_ENUM(gdwcore::network::PeerConnectionDirection, (unknown)
    (inbound)
    (outbound))
    FC_REFLECT_ENUM(gdwcore::network::FirewalledState, (unknown)
    (firewalled)
    (not_firewalled))

    FC_REFLECT(gdwcore::network::CurrentTimeRequestMessage, (request_sent_time))
    FC_REFLECT(gdwcore::network::CurrentTimeReplyMessage, (request_sent_time)
    (request_received_time)
    (reply_transmitted_time))
    FC_REFLECT_ENUM(gdwcore::network::FirewallCheckResult, (unable_to_check)
    (unable_to_connect)
    (connection_successful))
    FC_REFLECT(gdwcore::network::CheckFirewallMessage, (node_id)(endpoint_to_check))
    FC_REFLECT(gdwcore::network::CheckFirewallReplyMessage, (node_id)(endpoint_checked)(result))
    FC_REFLECT_EMPTY(gdwcore::network::GetCurrentConnectionsRequestMessage)
    FC_REFLECT(gdwcore::network::CurrentConnectionData, (connection_duration)
    (remote_endpoint)
    (node_id)
    (clock_offset)
    (round_trip_delay)
    ...
    (firewalled)
    (user_data))
    FC_REFLECT(gdwcore::network::GetCurrentConnectionsReplyMessage, (upload_rate_one_minute)
...
{
    template<>
    struct hash < gdwcore::network::ItemId >
    {
        size_t operator()(const gdwcore::network::ItemId& item_to_hash) const
        {
