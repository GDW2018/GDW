...
namespace gdwcore {
    namespace client {
        ...
        namespace detail {
            ...
            {
            public:
                typedef gdwcore::db::LevelMap<fc::time_point, fc::exception> ExceptionLeveldbType;
            private:
            ...
            public:
                void open(const fc::path& filename)
                {
                    ...
                    catch (const gdwcore::db::level_map_open_failure&)
                    {
            ...
            class ClientImpl : public gdwcore::network::NodeDelegate,
                public gdwcore::api::CommonApi
            {
            public:
                ...
                ClientImpl(gdwcore::client::Client* self, const std::string& user_agent) :
                    _self(self),
                    ...
                    _blockchain_synopsis_size_limit((unsigned)(log2(GDW_BLOCKCHAIN_BLOCKS_PER_YEAR * 20))),
                    _debug_last_wait_block(0)
                {
                    try
                    {
                        if (uvm::lua::api::global_uvm_chain_api == nullptr)
                            uvm::lua::api::global_uvm_chain_api = new gdwcore::lua::api::UvmChainApi;
                ...
                */
				...
				gdwcore::consensus::TransactionIdType network_broadcast_transactions(const std::vector<gdwcore::consensus::SignedTransaction>& transactions_to_broadcast);
                ...
                // @{
				virtual bool has_item(const gdwcore::network::ItemId& id) override;
				fc::time_point_sec handle_ntp_time(bool need_update)override;
                virtual bool handle_message(const gdwcore::network::Message&, bool sync_mode) override;
                virtual std::vector<gdwcore::network::ItemHashType> get_item_ids(uint32_t item_type,
                    const vector<gdwcore::network::ItemHashType>& blockchain_synopsis,
                    uint32_t& remaining_item_count,
                    uint32_t limit = 2000) override;
                virtual gdwcore::network::Message get_item(const gdwcore::network::ItemId& id) override;
                virtual fc::sha256 get_chain_id() const override
                {
                    FC_ASSERT(_chain_db != nullptr);
                    return _chain_db->get_chain_id();
                }
                virtual std::vector<gdwcore::network::ItemHashType> get_blockchain_synopsis(uint32_t item_type,
                ...
                virtual uint32_t get_block_number(const gdwcore::network::ItemHashType& block_id) override;
                virtual fc::time_point_sec get_block_time(const gdwcore::network::ItemHashType& block_id) override;
                virtual fc::time_point_sec get_blockchain_now() override;
                virtual gdwcore::network::ItemHashType get_head_block_id() const override;
                ...
                gdwcore::client::Client*                                    _self;
                std::string                                             _user_agent;
                gdwcore::console::Cli*                                          _cli = nullptr;
                ...
                gdwcore::rpc::RpcServerPtr                                _rpc_server = nullptr;
                //std::unique_ptr<gdwcore::network::ChainServer>                 _chain_server = nullptr;
                std::unique_ptr<gdwcore::network::UpnpService>                 _upnp_service = nullptr;
                ...
                //  std::shared_ptr<gdwcore::mail::server>                      _mail_server = nullptr;
                // std::shared_ptr<gdwcore::mail::client>                      _mail_client = nullptr;
                ...
                gdwcore::network::NodePtr                                      _p2p_node = nullptr;
                bool                                                    _simulated_network = false;

                GdwGntpNotifierPtr                                   _notifier;
                ...
                int save_code_to_file(const string& name, UvmModuleByteStream *stream, char* err_msg) const;
                Address get_contract_address(const string& contract, ChainInterfacePtr db=nullptr) const;


                ChainInterfacePtr simulator_get_correct_state_ptr() const;
                Address simulator_get_contract_address(const string& contract) const;
				fc::path contract_compile_helper(const fc::path& filename) const;

                boost::signals2::scoped_connection   _http_callback_signal_connection;

                //-------------------------------------------------- JSON-RPC Method Implementations
                // include all of the method overrides generated by the gdw_api_generator
