...
FC_REFLECT_ENUM(gdwcore::consensus::AccountType,
    (titan_account)
    (public_account)
    (multisig_account)
    )
    FC_REFLECT(gdwcore::consensus::MultisigMetaInfo,
    (required)
    (owners)
    )
    FC_REFLECT(gdwcore::consensus::AccountMetaInfo,
    (type)
    (data)
    )
    FC_REFLECT(gdwcore::consensus::DelegateStats,
    (votes_for)
    (pay_rate)
    (signing_key_history)
    ...
    FC_REFLECT(gdwcore::consensus::VoteDel,
    (votes)
    (delegate_id)
    )
    FC_REFLECT(gdwcore::consensus::AccountEntry,
    (id)
    (name)
    (public_data)
    (owner_key)
    ...
    FC_REFLECT(gdwcore::consensus::DelegatePaySalary, (total_balance)(pay_balance))
