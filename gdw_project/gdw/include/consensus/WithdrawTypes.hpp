...
namespace gdwcore {
    namespace consensus {
        ...
        struct MemoData
        {
             ...
             */
            fc::array<char, GDW_BLOCKCHAIN_MAX_MEMO_SIZE>     message;
        ...
        struct ExtendedMemoData
        {
            ...
            fc::array<char, GDW_BLOCKCHAIN_MAX_MEMO_SIZE>      message;
            fc::enum_type<uint8_t, MemoFlagsEnum>            memo_flags;
            fc::array<char, GDW_BLOCKCHAIN_EXTENDED_MEMO_SIZE> extra_memo;
        };
...
namespace fc {
    void to_variant(const gdwcore::consensus::WithdrawCondition& var, variant& vo);
    void from_variant(const variant& var, gdwcore::consensus::WithdrawCondition& vo);
    void to_variant(const gdwcore::consensus::MemoData& var, variant& vo);
    void from_variant(const variant& var, gdwcore::consensus::MemoData& vo);
}

FC_REFLECT_ENUM(gdwcore::consensus::WithdrawConditionTypes,
    (withdraw_null_type)
    (withdraw_signature_type)
    (withdraw_multisig_type)
    (withdraw_escrow_type)
    )
    FC_REFLECT_ENUM(gdwcore::consensus::WithdrawBalanceTypes,
    (withdraw_common_type)
    (withdraw_contract_type)
    (withdraw_margin_type)
    )
    FC_REFLECT(gdwcore::consensus::WithdrawCondition,
    (asset_id)
    (slate_id)
    (type)
    (balance_type)
    ...
    FC_REFLECT(gdwcore::consensus::TransferMemo,
    (one_time_key)
    (encrypted_memo_data)
    )
    FC_REFLECT_ENUM(gdwcore::consensus::MemoFlagsEnum,
    (from_memo)
    (to_memo)
    )
    FC_REFLECT(gdwcore::consensus::MemoData,
    (from)
    (from_signature)
    (message)
    (memo_flags)
    ...
    FC_REFLECT(gdwcore::consensus::ExtendedMemoData,
    (from)
    (from_signature)
    (message)
    (memo_flags)
    ...
    FC_REFLECT_DERIVED(gdwcore::consensus::MemoStatus,
    (gdwcore::consensus::MemoData),
    (has_valid_signature)
    (owner_private_key)
    )
    FC_REFLECT(gdwcore::consensus::WithdrawWithSignature,
    (owner)
    (memo)
    )

    FC_REFLECT(gdwcore::consensus::WithdrawWithMultisig,
    (required)
    (owners)
    (memo)
    )
    FC_REFLECT(gdwcore::consensus::withdraw_with_escrow,
    (sender)
    (receiver)
    (escrow)
    (agreement_digest)
