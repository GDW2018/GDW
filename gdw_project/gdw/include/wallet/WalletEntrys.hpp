...
#include <consensus/balance/BalanceEntry.hpp>
#include <consensus/address/ExtendedAddress.hpp>
...
namespace gdwcore {
    namespace wallet {
        ...
        enum PropertyEnum
        {
            ...
            last_scanned_block_number_for_gdwcore
        };
        ...
        struct AccountData : public gdwcore::consensus::AccountEntry
        {
        ...
        struct ScriptEntryPrintable
        {
            string id;				//脚本id
            bool enable;				//启用状态
			uvm::blockchain::CodePrintAble code_printable;			//代码
            string description;			//备注
            fc::time_point_sec  register_time;
...
{
    template<>
    struct hash < gdwcore::wallet::ScriptRelationKey >
    {
        size_t operator()(const gdwcore::wallet::ScriptRelationKey& s)const
        {
...
FC_REFLECT_ENUM(gdwcore::wallet::WalletEntryTypeEnum,
    ...
    FC_REFLECT_ENUM(gdwcore::wallet::PropertyEnum,
    (version)
    (next_entry_number)
    (next_child_key_index)
    (automatic_backups)
...
(last_scanned_block_number_for_gdwcore)
    ...
    FC_REFLECT(gdwcore::wallet::WalletProperty,
    (key)
    (value)
    )

    FC_REFLECT_DERIVED(gdwcore::wallet::AccountData, (gdwcore::consensus::AccountEntry),
    (is_my_account)
    (approved)
    (is_favorite)
    ...
    FC_REFLECT_DERIVED(gdwcore::wallet::AccountAddressData, (gdwcore::wallet::AccountData),
    (owner_address)
    (active_address)
    )

    FC_REFLECT(gdwcore::wallet::MasterKey,
    (encrypted_key)
    (checksum)
    )

    FC_REFLECT(gdwcore::wallet::KeyData,
    (account_address)
    (public_key)
    ...
    FC_REFLECT_ENUM(gdwcore::wallet::ContactData::ContactTypeEnum,
    (account_name)
    (public_key)
    (address)
    (btc_address)
    ...
    FC_REFLECT(gdwcore::wallet::ContactData,
    (contact_type)
    (data)
    (label)
    (favorite)
    ...
    FC_REFLECT(gdwcore::wallet::TransactionData,
    (entry_id)
    (block_num)
    (is_virtual)
    (is_confirmed)
    ...
    FC_REFLECT(gdwcore::wallet::TransactionLedgerEntry,
    (id)
    (block_num)
    (block_id)
    (timestamp)
    ...
    FC_REFLECT_DERIVED(gdwcore::wallet::PrettyTransactionExperimental, (gdwcore::wallet::TransactionLedgerEntry),
    (inputs)
    (outputs)
    (balances)
    (notes)
    ...
    FC_REFLECT(gdwcore::wallet::Setting,
    (name)
    (value)
    )

    FC_REFLECT(gdwcore::wallet::GenericWalletEntry,
    (type)
    (data)
    )

    FC_REFLECT(gdwcore::wallet::ScriptEntry,
    (id)
    (enable)
    (code)
    (description)
    ...
    FC_REFLECT(gdwcore::wallet::ScriptEntryPrintable,
    (id)
    (enable)
    (code_printable)
    (description)
    ...
    FC_REFLECT(gdwcore::wallet::ScriptRelationKey,
    (contract_id)
    (event_type)
    )
...
namespace fc {
    ...
    template<typename T, gdwcore::wallet::WalletEntryTypeEnum N>
    struct get_typename < gdwcore::wallet::WalletEntry<T, N> >
    {
    ...
    template<typename Type, gdwcore::wallet::WalletEntryTypeEnum N>
    struct reflector < gdwcore::wallet::WalletEntry<Type, N> >
    {
        typedef gdwcore::wallet::WalletEntry<Type, N>  type;
        ...
        {
            {
                typedef decltype(((gdwcore::wallet::BaseEntry<N>*)nullptr)->wallet_entry_index) member_type;
                visitor.TEMPLATE operator() < member_type, gdwcore::wallet::BaseEntry<N>, &gdwcore::wallet::BaseEntry<N>::wallet_entry_index > ("index");
            }
