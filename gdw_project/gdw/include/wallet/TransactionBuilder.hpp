...
namespace gdwcore {
    namespace wallet {
        ...
        struct TransactionBuilder {
            ...
            std::unordered_set<gdwcore::consensus::Address>                                      required_signatures;

            ///Map of <owning account address, asset ID> to that account's balance in that asset ID
            std::map<std::pair<gdwcore::consensus::Address, AssetIdType>, ShareType>          outstanding_balances;

            ///Map of account address to key owning that account's market transactions
            std::map<gdwcore::consensus::Address, PublicKeyType>                               order_keys;
             ...
             */
            PublicKeyType order_key_for_account(const gdwcore::consensus::Address& account_address, const string& account_name);
             ...
             */
            /// @{
            void deduct_balance(const gdwcore::consensus::Address& account_to_charge, const gdwcore::consensus::Asset& amount)
            {
            ...
            void credit_balance(const gdwcore::consensus::Address& account_to_credit, const gdwcore::consensus::Asset& amount)
            {
...
} //namespace gdwcore::wallet

FC_REFLECT_ENUM(gdwcore::wallet::VoteStrategy,
    (vote_none)
    (vote_all)
    (vote_random)
    (vote_recommended)
    )
    FC_REFLECT(gdwcore::wallet::TransactionBuilder, (transaction_entry)(required_signatures)(outstanding_balances))
