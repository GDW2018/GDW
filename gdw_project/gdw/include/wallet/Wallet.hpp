...
#include <consensus/transaction/TransactionCreationState.hpp>
#include <wallet/Pretty.hpp>
#include <wallet/TransactionBuilder.hpp>
#include <wallet/WalletDb.hpp>

#include <fc/signals.hpp>
#define INVALIDE_SGDW_ADDRESS ("ffffffffffffffffffffffffffffffff")
namespace gdwcore {
    namespace wallet {
        ...
        public:
            ...
            */
            ...
            void					set_last_scanned_block_number_for_ub(uint32_t block_num);
            /**
            ...
            */
            ...
            uint32_t				get_last_scanned_block_number_for_ub()const;
           ...
           */
            ...
            *
            * @param wif_key A private key in gdwcore wallet import format
            ...
            * @param amount_to_transfer_symbol the asset to transfer like GDW 
            ...
            */
            ...
            WalletTransactionEntry contract_upgrade(
			...
			std::vector<gdwcore::consensus::Asset> contract_upgrade_testing(
				const Address& contract_id,
				const string& upgrader_name,
				const string& contract_name,
				const string& contract_desc,
            ...
            WalletTransactionEntry contract_destroy(
                const Address& contract_id,
			...
			std::vector<gdwcore::consensus::Asset> contract_destroy_testing(
				const Address& contract_id,
				const string& destroyer_name,
				bool sign = true
				);
            ...
            */
            WalletTransactionEntry register_account(
                ...
                gdwcore::consensus::AccountType new_account_type,
                bool sign
                );
            /**
            ...
            */
            DelegatePaySalary query_delegate_salary(
                const string& delegate_name);
            std::map<std::string, gdwcore::consensus::DelegatePaySalary> query_delegate_salarys();
            ...
            * @return string
            */
            PrettyTransaction				    to_pretty_trx(const gdwcore::consensus::TransactionEntry& trx_entry, const std::string addr_for_fee) const;


            PrettyContractTransaction           to_pretty_contract_trx(const gdwcore::consensus::TransactionEntry& trx_entry) const;
            ...
            */
            ...
            WalletTransactionEntry contract_register(const string& owner, const fc::path codefile, const string& asset_symbol, double init_limit, bool is_testing = false);
            std::vector<gdwcore::consensus::Asset> contract_register_testing(const string& owner, const fc::path codefile);
            ...
            std::vector<gdwcore::consensus::Asset> contract_call_testing(const string caller, const ContractIdType contract, const string method, const string& arguments);
			...
			void simulator_get_enough_balances(const string& account_name, const Asset target, std::map<BalanceIdType, ShareType>& balances, unordered_set<Address>& required_signatures);
			AccountBalanceEntrySummaryType simulator_get_spendable_account_balance_entries(const string& account_name);

            //simulator relate function
            ChainInterfacePtr get_correct_state_ptr() const;
			WalletDb& get_wallet_db() const;
            /*WalletTransactionEntry simulator_contract_register(const string& owner, const fc::path codefile, const string& asset_symbol, double init_limit);*/
			void scan_contracts();

            vector<ScriptEntry> scripts_list();

            oScriptEntry get_script_entry(const ScriptIdType&);
            ScriptIdType script_add(const fc::path& filename, const string& description = string(""));
            void delete_script(const ScriptIdType& script_id);
            void script_disable(const ScriptIdType& script_id);
            void script_enable(const ScriptIdType& script_id);
            void script_import_to_db(const fc::path& src_path);
            void script_export_from_db(const fc::path & des_path);
            vector<ScriptIdType> script_list_event_handler(const ContractIdType& contract_id, const std::string& event_type);
            void script_add_event_handler(const ContractIdType& contract_id, const std::string& event_type, const ScriptIdType& script_id, uint32_t index);
            void script_delete_event_handler(const ContractIdType& contract_id, const std::string& event_type, const ScriptIdType& script_id);
			std::vector<std::string> script_get_events_bound(const std::string& script_id);
        private:
...
FC_REFLECT_ENUM(gdwcore::wallet::AccountKeyType,
    (owner_key)
    (active_key)
    (signing_key)
    )
