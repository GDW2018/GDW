...
namespace gdwcore {
    namespace rpc {
        ...
        /**
        *  @class rpc_server
        *  @brief provides a json-rpc interface to the gdwcore client
        */
        class RpcServer
        {
        public:
            RpcServer(gdwcore::client::Client* client);
            virtual ~RpcServer();

            bool        configure_rpc(const gdwcore::client::RpcServerConfig& cfg);
            bool        configure_http(const gdwcore::client::RpcServerConfig& cfg);
            bool        configure_encrypted_rpc(const gdwcore::client::RpcServerConfig& cfg);
            ...
            const gdwcore::api::MethodData& get_method_data(const std::string& method_name);
            std::vector<gdwcore::api::MethodData> get_all_method_data() const;
        ...
        protected:
            friend class gdwcore::rpc::detail::RpcServerImpl;

            void validate_method_data(gdwcore::api::MethodData method);
            void register_method(gdwcore::api::MethodData method);
